-- Modern showroom - esx_boat client side
...
print('^2[esx_boat]^7 client/showroom.lua loaded - modern showroom UI is available')
...
function CloseBoatShopNUI(resetPed)
	ShowroomOpen = false
...
RegisterNUICallback('close', function(_, cb) CloseBoatShopNUI(true); cb('ok') end)
...
RegisterNUICallback('buyVehicle', function(data, cb)
	...
	ESX.Game.SpawnVehicle(model, shop.Outside, GetEntityHeading(playerPed), function(vehicle)
		...
		TriggerServerEvent('esx_boat:completePurchase', vehicleProps)
...
RegisterNetEvent('esx_boat:purchaseSucceeded')
AddEventHandler('esx_boat:purchaseSucceeded', function(name, price)
	ESX.ShowNotification(('%s خریداری شد. ($%s)'):format(name, ESX.Math.GroupDigits(price)))
end)

RegisterNetEvent('esx_boat:purchaseFailed')
AddEventHandler('esx_boat:purchaseFailed', function(reason)
