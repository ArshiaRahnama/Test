-- ============================================================================
-- ✅ ادغام شد: این فایل قبلاً ریسورس جدای «Unique_LogPanel» بود؛ حالا بخشی از
-- همین ریسورس «logs» ـه (چون همیشه فقط از جدول unique_logpanel که پایین‌تر همین
-- ریسورس (SERVER/Server.lua -> SendToSite) پر می‌کرد می‌خوند، نگه‌داشتنش به‌صورت
-- ریسورس جدا فقط یه وابستگی/ترتیب‌بارگذاری اضافه بدون فایده بود). هیچ منطقی از
-- این فایل عوض نشده — فقط جابه‌جا شده؛ export ها و fetch سمت NUI (که از
-- GetParentResourceName() استفاده می‌کنه) خودکار با اسم ریسورس جدید (logs) کار
-- می‌کنن، بدون نیاز به تغییر تو html/index.html یا exports['Unique_LogPanel'].
-- ============================================================================
