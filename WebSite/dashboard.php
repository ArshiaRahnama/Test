...
function ticket_of(int $id, array $u): ?array {
  $s = db()->prepare('SELECT * FROM tickets WHERE id=?'); $s->execute([$id]); $t = $s->fetch();
  return ($t && ($t['user_id'] == $u['id'] || $u['role'] === 'admin')) ? $t : null;
}
function add_msg(int $tid, int $uid, string $b): void {
  db()->prepare('INSERT INTO msgs(ticket_id,user_id,body,created) VALUES(?,?,?,?)')->execute([$tid, $uid, mb_substr($b, 0, 2000), time()]);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($a === 'new') {
    ...
    if ($s === '' || $b === '' || mb_strlen($s) > 120) { $flash = 'موضوع و متن را کامل بنویس.'; $p = 'tickets'; }
      ...
      $db->prepare('INSERT INTO tickets(user_id,subject,status,created) VALUES(?,?,?,?)')->execute([$u['id'], $s, 'open', time()]);
    ...
    if ($t) {
      if ($a === 'close') $db->prepare("UPDATE tickets SET status='closed' WHERE id=?")->execute([$id]);
      elseif (trim($_POST['body'] ?? '') !== '' && $t['status'] !== 'closed') {
        add_msg($id, $u['id'], trim($_POST['body']));
        $db->prepare('UPDATE tickets SET status=? WHERE id=?')->execute([$admin && $t['user_id'] != $u['id'] ? 'answered' : 'open', $id]);
      }
    ...
    else { $db->prepare('UPDATE users SET pass=? WHERE id=?')->execute([password_hash($_POST['new'], PASSWORD_DEFAULT), $u['id']]); $flash = 'رمز عبور تغییر کرد.'; }
  }
}
...
<?php elseif ($p === 'tickets'): ?>
  ...
  <?php $q = $db->prepare('SELECT t.*,u.fullname FROM tickets t JOIN users u ON u.id=t.user_id ' . ($admin ? '' : 'WHERE t.user_id=? ') . 'ORDER BY t.id DESC LIMIT 100');
...
<?php elseif ($p === 'ticket' && ($t = ticket_of((int)($_GET['id'] ?? 0), $u))): ?>
  ...
  <?php $m = $db->prepare('SELECT m.*,u.fullname,u.role FROM msgs m JOIN users u ON u.id=m.user_id WHERE ticket_id=? ORDER BY m.id'); $m->execute([$t['id']]); ?>
