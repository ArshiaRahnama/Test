-------------------------------------------------------------------
-- Generic "stash" (shared/ground storage) client for esx_inventory.
--
-- esx_inventory doesn't ship a shared-storage container out of the box
...
function LoadStashData(stashId, maxWeight, label, cb)
    TriggerServerCallback("esx_inventory:getStash", function(data)
...
function OpenStash(stashId, maxWeight, slot, label)
    ...
    TriggerServerEvent('esx_inventory:stashViewer', stashId, true)
...
RegisterNetEvent('esx_inventory:stashUpdated')
AddEventHandler('esx_inventory:stashUpdated', function(stashId)
    if CurrentStashId == stashId then
...
function HandleStashPut(data, cb)
    ...
    if item.type == 'item_weapon' then
        if not Config.WeaponNoGive[item.name] then
            ...
            TriggerServerEvent('esx_inventory:stashDeposit', CurrentStashId, 'item_weapon', item.name, 1, item.serial)
        end
    elseif item.type == 'item_account' then
        KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result)
            if result ~= nil and tonumber(result) then
                TriggerServerEvent('esx_inventory:stashDeposit', CurrentStashId, 'item_account', item.name, tonumber(result))
            end
        end)
    elseif item.type == 'item_standard' then
        KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result)
            if result ~= nil and tonumber(result) then
                TriggerServerEvent('esx_inventory:stashDeposit', CurrentStashId, 'item_standard', item.name, tonumber(result))
            end
        end)
    end
...
function HandleStashTake(data, cb)
    ...
    if item.type == 'item_weapon' then
        if not Config.WeaponNoGive[item.name] then
            TriggerServerEvent('esx_inventory:stashWithdraw', CurrentStashId, 'item_weapon', item.name, 1, item.serial)
        end
    elseif item.type == 'item_account' then
        KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result)
            if result ~= nil and tonumber(result) then
                TriggerServerEvent('esx_inventory:stashWithdraw', CurrentStashId, 'item_account', item.name, tonumber(result))
            end
        end)
    elseif item.type == 'item_standard' then
        KeyboardUtils.use(Locales[Config.Language]['quantite'], function(result)
            if result ~= nil and tonumber(result) then
                TriggerServerEvent('esx_inventory:stashWithdraw', CurrentStashId, 'item_standard', item.name, tonumber(result))
            end
        end)
    end
