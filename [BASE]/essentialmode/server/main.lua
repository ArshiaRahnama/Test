...
-- ================================================================= --
-- Single source of truth for a player's max carry weight: base +
-- level bonus (Unique_LevelQuest's rank) + backpack bonus (equipped
-- backpack item, if any). Called from three places that each know
-- about one piece of this and shouldn't have to re-derive the whole
-- formula: server/player/login.lua (initial spawn value), the
-- levelChanged handler right below (level up/down mid-session), and
-- esx_inventory's backpack equip/unequip flow (server/custom/apps/backpack.lua).
-- rank/backpackName are optional - pass nil to keep using whatever is
-- already on the player object (self.rank / self.equippedBackpack).
-- ================================================================= --
function RecalculateMaxWeight(xPlayer, rank, backpackName)
    if not xPlayer then return end
    if rank ~= nil then
        xPlayer.rank = math.max(1, tonumber(rank) or 1)
    end
    if backpackName ~= nil then
        xPlayer.equippedBackpack = backpackName ~= '' and backpackName or nil
    end

    local effectiveRank = math.max(1, tonumber(xPlayer.rank) or 1)
    local levelBonus = (effectiveRank - 1) * (Config.WeightPerLevel or 5)
    local backpackBonus = 0
    if xPlayer.equippedBackpack and Config.BackpackWeight and Config.BackpackWeight[xPlayer.equippedBackpack] then
        backpackBonus = Config.BackpackWeight[xPlayer.equippedBackpack]
    end

    xPlayer.setMaxWeight((Config.DefaultMaxWeight or 24) + levelBonus + backpackBonus)
end

-- ================================================================= --
-- Live inventory-capacity update when Unique_LevelQuest levels a
-- player up/down mid-session (see server/xp.lua in that resource -
-- it fires this after every rank change). Initial capacity at spawn
-- is set from the DB directly in server/player/login.lua; this just
-- keeps it in sync without requiring a relog.
-- ================================================================= --
-- Equip/unequip a backpack (backpackName = nil or '' to remove). Persists
-- to the users.equipped_backpack column and immediately recalculates
-- carry capacity. Exported so esx_inventory's backpack item handler
-- doesn't need to know the DB column name or touch xPlayer internals.
function SetPlayerBackpack(source, backpackName)
    local xPlayer = ESX.GetPlayerFromId(tonumber(source))
    if not xPlayer then return false end
    backpackName = (backpackName ~= nil and backpackName ~= '') and backpackName or nil

    MySQL.Async.execute('UPDATE users SET equipped_backpack = @backpack WHERE identifier = @identifier', {
        ['@identifier'] = xPlayer.identifier,
        ['@backpack'] = backpackName,
    })
    RecalculateMaxWeight(xPlayer, nil, backpackName or '')
    return true
end
exports('SetPlayerBackpack', SetPlayerBackpack)

function GetPlayerBackpack(source)
    local xPlayer = ESX.GetPlayerFromId(tonumber(source))
    if not xPlayer then return nil end
    return xPlayer.equippedBackpack
end
exports('GetPlayerBackpack', GetPlayerBackpack)

AddEventHandler('essentialmode:levelChanged', function(playerId, rank)
    local xPlayer = ESX.GetPlayerFromId(tonumber(playerId))
    if not xPlayer then return end
    RecalculateMaxWeight(xPlayer, rank)
end)
