...
-- FIX: these used to unconditionally return 0 for every item/weapon, no
-- matter what. Effects of that: getUsedWeight() (player.lua) was always
-- 0, so the weight bar/cap never actually enforced anything, AND every
-- caller that reads an item's `.weight` field directly instead of
-- calling this function (e.g. esx_inventory's trunk deposit: `InfoItem
-- .weight * count`) got `nil * count` and crashed outright, because
-- nothing ever populated that field either. Now backed by the `items`
-- table's `weight` column (falls back to Config.DefaultItemWeight for
-- any item that doesn't have one set), and Config.WeaponWeight for
-- weapons.
ESX.getItemWeight = function(name)
    ...
    return Config.DefaultItemWeight or 0.5
end

ESX.getWeaponWeight = function(name)
    if Config.WeaponWeight and Config.WeaponWeight[name] then
        return Config.WeaponWeight[name]
    end
    return Config.WeaponDefaultWeight or 2
end
...
function RegisterItem(name, label, weight)
    if type(name) ~= 'string' or name == '' then return false end
    weight = tonumber(weight) or Config.DefaultItemWeight or 0.5
    if not ESX.Items[name] then
        ESX.Items[name] = { name = name, label = label or name, limit = -1, rare = false, canRemove = true, weight = weight }
        MySQL.Async.execute('INSERT IGNORE INTO items (name, label, `limit`, rare, can_remove, weight) VALUES (?, ?, -1, 0, 1, ?)', { name, label or name, weight })
    end
    return true
end
...
-- exactly why every custom cross-resource callback (esx_inventory's
...
-- Unique_ALLGangs/server/apps/system/stash.lua (esx_inventory).
            ...
            function(result)
                for i = 1, #result, 1 do
                    ESX.Items[result[i].name] = {
                        ...
                        -- FIX: `weight` column may not exist yet on servers that
                        -- haven't run the migration in database.sql, and NULL is
                        -- valid even once it does (existing rows) - either way
                        -- fall back to the configured default instead of storing
                        -- a nil/0 weight for the item.
                        weight = tonumber(result[i].weight) or Config.DefaultItemWeight or 0.5
                    }
                end
            end
        )
