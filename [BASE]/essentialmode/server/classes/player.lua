...
function CreatePlayer(
    ...
    -------------------------------------------------------------
    -- FIX (one-time cleanup of stale corrupted data): confirmed via
    -- console log that some players have loadout entries with
    -- serial=nil sitting in their saved data - leftover artifacts from
    -- before addWeapon always generated a serial. Every weapon added
    -- now always gets a real serial, so a nil one can only be this old
    -- corrupted state, never a legitimate current entry - safe to
    -- strip on load without touching any real, distinct, serialed
    -- weapon instance (including legitimate duplicates of the same
    -- weapon type with different serials, which are left alone).
    -------------------------------------------------------------
    do
        local cleaned, removedCount = {}, 0
        for _, w in ipairs(self.loadout) do
            if w.serial ~= nil then
        ...
        if removedCount > 0 then
            print(('[essentialmode] player load: stripped %d serial-less (corrupted) loadout entr%s for identifier %s'):format(
                removedCount, removedCount == 1 and 'y' or 'ies', tostring(self.identifier)))
            self.loadout = cleaned
        end
    end
    ...
    if ESX.DoesJobExist(job, jgrade) then
		...
		if jobObject.name == 'police' or jobObject.name == 'sheriff' or jobObject.name == 'fbi' or jobObject.name == 'mt' then
			...
			local wai = true
			exports.oxmysql:execute('SELECT * FROM police_ext WHERE identifier = @identifier',{
				['identifier'] = self.identifier
			},
			function(result)
				if result[1] then
    ...
    self.canAfford = function(amount)
        if type(amount) ~= "number" or amount <= 0 then return false end
        return (self.money + self.bank) >= amount
    end

    -- pays `amount` using cash first, then pulls the remainder from bank.
    -- returns true/false depending on whether the player could afford it.
    self.payAny = function(amount)
        if type(amount) ~= "number" or amount <= 0 then return true end
        if not self.canAfford(amount) then return false end

        local fromCash = math.min(self.money, amount)
        local fromBank = amount - fromCash

        if fromCash > 0 then
            self.removeMoney(fromCash)
        end

        if fromBank > 0 then
            self.removeBank(fromBank)
        end
    ...
    self.getInventoryItem = function(name)
        ...
        return {
            ...
            -- FIX: this stub never carried a weight field, so anything
            -- reading `.weight` straight off an item (e.g. esx_inventory's
            -- trunk deposit: `InfoItem.weight * count`) got nil and
            -- crashed. ESX.getItemWeight() already has its own fallback,
            -- reuse it here instead of duplicating the fallback logic.
            weight = ESX.getItemWeight(name)
        }
    end
