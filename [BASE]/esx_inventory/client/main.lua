...
function closeInventory()
    ...
    if CurrentStashId then
        Inv.isInTrunk = false
        TriggerServerEvent('esx_inventory:stashViewer', CurrentStashId, false)
        CurrentStashId = nil
...
-- FIX (requested): this used to unconditionally call BlockWeaponWheelThisFrame()
-- and SetPedCanSwitchWeapon(ped, false) in the loop above, every tick, all the
-- time - not just while the inventory UI was actually open. That's why the
-- default GTA weapon wheel (hold Tab) never worked at all, for any weapon,
...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(0)
        local invOpen = Inv.isInInventory or Inv.isInTrunk or Inv.openInvPlayer or Inv.isInProperty
        if invOpen then
...
RegisterCommand('inventory', function()
    ...
    elseif Inv.isInInventory or Inv.openInvPlayer then
        -- FIX (requested): F2 used to only ever open the inventory - this
        -- branch existed already but was commented out, so pressing F2
        -- again while it was open did nothing. Restored so F2 toggles
        -- open/close like Esc already does (or now does, see the Esc
        -- keydown handler added in inventory.js). Left scoped to the
        -- plain player inventory / other-player-inventory cases only,
        -- same as before - trunk/property have their own dedicated close
        -- flows and their own interaction to back out of.
        Inv.openInvPlayer = false
        closeInventory()
    end
end)
...
function removePedInv(bool)
    Inv.noPedInv = GetFieldValueFromName('esx_inventory').pedscreen
    if Inv.noPedInv == false or  Inv.noPedInv == nil then
        Inv.noPedInv = true
        SetFieldValueFromNameEncode('esx_inventory', {pedscreen = bool})
    else
        if Inv.noPedInv == true then
            Inv.noPedInv = false
            SetFieldValueFromNameEncode('esx_inventory', {pedscreen = bool})
...
Inv.FastWeapons = GetFieldValueFromName('esx_inventory').name and GetFieldValueFromName('esx_inventory').name or {}
...
function loadPlayerInventory(result, coffre, category, poid)
        TriggerServerCallback("lgddddd:getPlayerInventory", function(data)
            ...
            if data == nil then
                debugprint('loadPlayerInventory: getPlayerInventory returned nil - retrying in 1s')
