...
-- callback function 100% local to esx_inventory (it never crosses any
...
AddEventHandler('essentialmode:relayServerCallback:esx_inventory', function(name, requestId, source, ...)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    -- as any later restart of it while esx_inventory keeps running
...
function RegisterServerCallback(name, cb)
    ...
    if GetResourceState('essentialmode') == 'started' then
        if not pushRegistration(name) then
            print('[esx_inventory] RegisterServerCallback(' .. tostring(name) .. '): essentialmode rejected the relay registration - will keep retrying on the recurring timer')
        end
    else
        print('[esx_inventory] RegisterServerCallback(' .. tostring(name) .. '): essentialmode never reached "started" after 30s (currently: ' .. tostring(GetResourceState('essentialmode')) .. ') - will keep retrying on the recurring timer')
    end
end
