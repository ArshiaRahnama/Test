-------------------------------------------------------------------
-- Generic "stash" (shared/ground storage) server for esx_inventory.
--
-- Added because esx_inventory has no built-in shared-storage container
...
local function refreshStashViewers(stashId)
    ...
    for src in pairs(StashViewers[stashId]) do
        TriggerClientEvent('esx_inventory:stashUpdated', src, stashId)
    end
end
...
--   exports['esx_inventory']:registerStashAccessCheck('gang_ballas_armory')
-- and listen for 'esx_inventory:relayStashAccessCheck:<their resource
-- name>' to actually run the real check and reply on
-- 'esx_inventory:relayStashAccessCheckReply'.
...
RegisterServerEvent('esx_inventory:relayStashAccessCheckReply')
AddEventHandler('esx_inventory:relayStashAccessCheckReply', function(requestId, allowed)
...
local function canAccessStashItem(source, stashId, itemName)
    ...
    TriggerEvent('esx_inventory:relayStashAccessCheck:' .. owner, stashId, itemName, source, requestId)
...
RegisterServerCallback('esx_inventory:getStash', function(source, cb, stashId, maxWeight, label)
...
RegisterNetEvent('esx_inventory:stashViewer')
AddEventHandler('esx_inventory:stashViewer', function(stashId, opening)
    local source = source
...
RegisterNetEvent('esx_inventory:stashDeposit')
AddEventHandler('esx_inventory:stashDeposit', function(stashId, itemType, name, count, serial)
    local source = source
...
RegisterNetEvent('esx_inventory:stashWithdraw')
AddEventHandler('esx_inventory:stashWithdraw', function(stashId, itemType, name, count, serial)
    local source = source
    if type(stashId) ~= 'string' or type(name) ~= 'string' then return end
