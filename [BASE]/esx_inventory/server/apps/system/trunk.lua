...
RegisterNetEvent("lgdddd:actionItem", function(plate, class, action, count, name)
    ...
    -- FIX: GetItem/getInventoryItem returns nil for a name that isn't a
    -- registered item at all (as opposed to one the player just doesn't
    -- own, which comes back as a 0-count stub instead) - e.g. a tampered
    -- or stale client call. `InfoItem.count` right below used to hard-
    -- crash this whole event (and every trunk interaction after it, on
    -- some frameworks) the moment that happened.
    if not InfoItem then
        debugprint("BAN " .. source .. " (lgdddd:actionItem invalid item: " .. tostring(name) .. ")")
        return
    end
...
RegisterNetEvent("lgdddd:actionsAccount", function(action, vehicle, data)
    ...
    if action == "deposit" then
        if getAccount(xPlayer, data.type) >= tonumber(data.count) then
            if not VerifExistPlate then
                local CreateVeh = CreateVehToBdd(vehicle.plate, vehicle.class)
                if CreateVeh then
                    -- FIX: this branch treated the accounts entry as a bare
                    -- number (`... = ... + tonumber(data.count)`) instead of
                    -- the {count=...} table every other branch here uses -
                    -- harmless in practice today only because CreateVehToBdd
                    -- always starts a brand-new trunk with an empty accounts
                    -- table (so this ~= nil check can never actually be true
                    -- yet), but it's a landmine for whenever that changes.
    ...
    elseif action == "remove" then
        ...
        -- FIX: no existence check at all before this - withdrawing from an
        -- account that was never deposited into this trunk (or a trunk that
        -- doesn't exist yet, VerifExistPlate false) indexed straight into a
        -- nil table and crashed the whole event. Same pattern already used
        -- for weapons/items above.
        local account = VerifExistPlate and VehCoffre[vehicle.plate].data["accounts"][data.type]
        if not account then
        ...
        if account.count >= tonumber(data.count) then
            account.count = account.count - tonumber(data.count)
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------
-- Read-only export added for K9 script integration.
-- Returns the live (in-memory) list of items currently sitting in a vehicle's
-- trunk, or nil if the vehicle has no trunk data loaded. Does not modify
-- VehCoffre in any way, so it's safe alongside the rest of this file.
-- esx_inventory has no glovebox concept, so there is no equivalent export for it.
exports('GetTrunkItems', function(plate)
    if VehCoffre[plate] and VehCoffre[plate].data and VehCoffre[plate].data["items"] then
        local items = {}
        for name, item in pairs(VehCoffre[plate].data["items"]) do
            items[#items + 1] = { name = name, count = item.count }
        end
        return items
    end
    return nil
