...
-- uniquecafejobs: none of these 21 jobs (17 businesses + 4 holdings)
-- should ever see weapon, heli, or division management -- they're cafes
-- and logistics companies, not armed factions.
local uwuNoWeaponJobs = {
	uwucafe = true, obsidian = true, voltage = true, ember = true, anchor = true, crimson = true,
	flourish = true, goldcrust = true, carwash = true,
	firebrick = true, slice = true, frostbite = true, sundae = true,
	static = true, nightjar = true, koi = true, wasabi = true,
...
function OpenBossMenu(society, close, options)
	...
	if options.job and ESX.PlayerData.job.grade >= 10 then
		if ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'sheriff' or ESX.PlayerData.job.name == 'mt' then
			...
			table.insert(elements, {label = _U('manage_job'), value = 'manage_job'})
			if not uwuNoWeaponJobs[ESX.PlayerData.job.name] then
				table.insert(elements, {label = _U('manage_job_division'), value = 'manage_job_division'})
			end
		end
	end
...
function OpenManageJobMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:GetPermWashMoney', function(Wash)
		...
		-- uniquecafejobs: see uwuNoWeaponJobs at top of file.
