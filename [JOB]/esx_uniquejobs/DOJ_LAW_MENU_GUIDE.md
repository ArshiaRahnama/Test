...
5. **Mugshot** *(بازسازی‌شده)*:
   - **Gereftan Aks Jadid** -- اگه شهروند همون لحظه آنلاین باشه، یک دوربین
     ...
     از یک صدای شاتر، به‌صورت پیش‌فرض عکس مستقیم به‌صورت base64 توی خود
     دیتابیس ذخیره می‌شه (بدون نیاز به هیچ هاست بیرونی)؛ اگه بخوای، با
     convar `mugshot_upload_url` می‌تونی به‌جاش از یک هاست عکس بیرونی
     استفاده کنی (اختیاری `mugshot_upload_field` برای اسم فیلد فرم آپلود).
     اگه شهروند آفلاین باشه یا `screenshot-basic` نصب نباشه، یک URL دستی
