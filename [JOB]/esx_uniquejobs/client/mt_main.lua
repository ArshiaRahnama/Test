...
-- (locales/mt_en.lua -> Locales['en_mt']) instead of the old shared global
...
local function _U(str, ...)
	local tbl = Locales['en_mt']
	local v = tbl and tbl[str]

	if ( v == nil ) then
		return "Error [en_mt][" .. tostring(str) .. "] Be Developer Elam Konid"
	end
...
-- Vehicles added live via esx_society's /addcarjob command (job = mt) get merged
...
local function removeInjected_mt(list, injected)
...
Citizen.CreateThread(function()
	...
	local function mergeCustomVehicles()
		ESX.TriggerServerCallback('esx_society:getCustomVehicles', function(list)
			removeInjected_mt(Config_mt.AuthorizedVehicles.Shared, InjectedVehicleEntries)
			removeInjected_mt(Config_mt.AuthorizedVehicles.Sharedheli, InjectedHeliEntries)
			InjectedVehicleEntries = {}
			InjectedHeliEntries    = {}
			CustomVehicleLiveries  = {}

			local custom = list and list['mt']
			...
			for _, v in ipairs(custom) do
				...
				if v.is_heli == 1 then
					table.insert(Config_mt.AuthorizedVehicles.Sharedheli, entry)
					table.insert(InjectedHeliEntries, entry)
				else
					table.insert(Config_mt.AuthorizedVehicles.Shared, entry)
