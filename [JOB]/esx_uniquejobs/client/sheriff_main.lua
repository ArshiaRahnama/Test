...
-- (locales/sheriff_en.lua -> Locales['en_sheriff']) instead of the old shared global
...
local function _U(str, ...)
	local tbl = Locales['en_sheriff']
	local v = tbl and tbl[str]

	if ( v == nil ) then
		return "Error [en_sheriff][" .. tostring(str) .. "] Be Developer Elam Konid"
	end
...
-- Vehicles added live via esx_society's /addcarjob command (job = sheriff) get merged
...
local function removeInjected_sheriff(list, injected)
...
Citizen.CreateThread(function()
	...
	local function mergeCustomVehicles()
		ESX.TriggerServerCallback('esx_society:getCustomVehicles', function(list)
			removeInjected_sheriff(Config_sheriff.AuthorizedVehicles.Shared, InjectedVehicleEntries)
			removeInjected_sheriff(Config_sheriff.AuthorizedVehicles.Sharedheli, InjectedHeliEntries)
			InjectedVehicleEntries = {}
			InjectedHeliEntries    = {}
			CustomVehicleLiveries  = {}

			local custom = list and list['sheriff']
			...
			for _, v in ipairs(custom) do
				...
				if v.is_heli == 1 then
					table.insert(Config_sheriff.AuthorizedVehicles.Sharedheli, entry)
					table.insert(InjectedHeliEntries, entry)
				else
					table.insert(Config_sheriff.AuthorizedVehicles.Shared, entry)
