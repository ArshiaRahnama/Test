...
-- (locales/marshal_en.lua -> Locales['en_marshal']) instead of the old shared global
...
local function _U(str, ...)
	local tbl = Locales['en_marshal']
	local v = tbl and tbl[str]

	if ( v == nil ) then
		return "Error [en_marshal][" .. tostring(str) .. "] Be Developer Elam Konid"
	end
...
-- Vehicles added live via esx_society's /addcarjob command (job = marshal) get merged
...
local function removeInjected_marshal(list, injected)
...
Citizen.CreateThread(function()
	...
	local function mergeCustomVehicles()
		ESX.TriggerServerCallback('esx_society:getCustomVehicles', function(list)
			removeInjected_marshal(Config_marshal.AuthorizedVehicles.Shared, InjectedVehicleEntries)
			removeInjected_marshal(Config_marshal.AuthorizedVehicles.Sharedheli, InjectedHeliEntries)
			InjectedVehicleEntries = {}
			InjectedHeliEntries    = {}
			CustomVehicleLiveries  = {}

			local custom = list and list['marshal']
			...
			for _, v in ipairs(custom) do
				...
				if v.is_heli == 1 then
					table.insert(Config_marshal.AuthorizedVehicles.Sharedheli, entry)
					table.insert(InjectedHeliEntries, entry)
				else
					table.insert(Config_marshal.AuthorizedVehicles.Shared, entry)
...
function OpenMarshalActionsMenu_marshal()
	...
	ESX.TriggerServerCallback('esx_society:divisionsPlayer', function(check)
		...
		elements = {
			...
			{label = 'Spawn Pet',			value = 'spawn_pet'},
		}
		...
		'default', GetCurrentResourceName(), 'marshal_actions',
		{
			...
			if data.current.value == 'spawn_pet' then
				if not HasAccess() then
					ESX.ShowNotification('~r~You don\'t have access to K9.')
					return
				end
				menu.close()
				OPEN_K9_MENU()
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config_marshal.MarshalStations) do
		...
		AddTextComponentString('Marshal Station')
