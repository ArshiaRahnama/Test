--[[
    kq_detective — server logic, adapted for this server:
      * core framework: essentialmode + ESX bridge (Config_detective.esxSettings.enabled)
      * revive hook: esx_ambulancejob:revivex ([JOB]/esx_uniquejobs)

    Player-death tracking used to only exist for QBCore (Config_detective.qbSettings.enabled).
...
if Config_detective.qbSettings.enabled then
    if Config_detective.qbSettings.useNewQBExport then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            RefreshDeadPlayers()
            Citizen.Wait(Config_detective.qbSettings.deadPlayerRefreshTime)
        end
    end)
