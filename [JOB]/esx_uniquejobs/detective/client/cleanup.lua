--[[
    ...
    NPC corpses only (see Config_detective.cleanup comment in config.lua for why player
...
AddEventHandler('kq_detective:collectBody', function(data)
    if not Config_detective.cleanup.enabled then return end
    if not Contains(Config_detective.cleanup.jobs, playerJob) then return end

    local ped = data.entity
    if not DoesEntityExist(ped) or not IsEntityDead(ped) or IsPedAPlayer(ped) then return end

    local coords = GetEntityCoords(ped)
    local bagModel = GetHashKey(Config_detective.cleanup.bagProp)
    local bag = nil
    ...
    Citizen.Wait(Config_detective.cleanup.bagDuration)
