function AddPedToTargetting(ped)
    if not (Config_detective.target.enabled and Config_detective.target.system) then return end

    local system = Config_detective.target.system
    local options = {
        {
            type = "client",
    ...
    if Config_detective.forensics.enabled then
    ...
    -- NPC-only (see config.lua's Config_detective.cleanup comment for why).
    if Config_detective.cleanup.enabled then
        table.insert(options, {
            type = "client",
            ...
            canInteract = function(entity)
                return IsEntityDead(entity) and not IsPedAPlayer(entity) and Contains(Config_detective.cleanup.jobs, playerJob)
            end,
        })
    end
