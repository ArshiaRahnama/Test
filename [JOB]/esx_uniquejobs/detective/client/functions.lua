--[[
    ...
    Fixes vs. the original file:
      1) CanInvestigate() was bugged: it read Config_detective.whitelist.enabled into a
      ...
      2) IsDead() / StartInvestigatePed() only recognised a player as
         "dead" when Config_detective.qbSettings.enabled was true. Since this server
...
function OpenNotepad(time, cause, isWeapon, weaponType, weaponName, vehicleClass, area, boneTop, boneLeft, samePed)
    ...
    if Config_detective.animation.enabled then
        PlayAnim(Config_detective.animation.dict, Config_detective.animation.anim, 1)
    end
...
-- Fixed: previously always returned true regardless of Config_detective.whitelist
-- (see file header). Now actually enforces the job whitelist.
function CanInvestigate()
    if not Config_detective.whitelist.enabled then
        return true
    end

    return Contains(Config_detective.whitelist.jobs, playerJob)
end
