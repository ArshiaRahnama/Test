    ...
    while true do
        ...
        if #deadPeds > 0 and not blocked then
            ...
            if closestPed and not menuOpen then
                if not Config_detective.target.enabled then
                    local coords = GetEntityCoords(closestPed)
                    wait = 0
                    local prompt = L('~w~[~y~{INVESTIGATE_KEYBIND}~w~] to investigate')
                        :gsub('{INVESTIGATE_KEYBIND}', Config_detective.keybinds.investigate)
                    Draw3DText(coords.x, coords.y, coords.z, prompt, 4, 0.035, 0.035)

                    if IsControlJustReleased(0, Keys[Config_detective.keybinds.investigate]) then
...
AddEventHandler('kq_detective:investigatePlayer', function(data, ped)
    ...
    if Config_detective.autopsy.enabled and data.distance and Contains(Config_detective.autopsy.jobs, playerJob) then
...
if Config_detective.qbSettings.enabled then
...
if Config_detective.esxSettings.enabled then
