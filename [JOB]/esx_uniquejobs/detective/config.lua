Config_detective = {}

-- TEMPORARILY true for solo testing — enables /kqtestcorpse, /kqtestautopsy,
-- /kqtestforensics and /kqtestbleedout (see server/*.lua + client/*.lua).
-- Set back to false before going live.
Config_detective.debug = true


--- SETTINGS FOR ESX
-- This server runs essentialmode with the ESX compatibility bridge
-- (see [BASE]/essentialmode/server/common.lua + client/common.lua, which
-- expose 'esx:getSharedObject'), so ESX mode is what we want enabled.
Config_detective.esxSettings = {
    enabled = true,
}

--- SETTINGS FOR QBCORE
-- Not used on this server.
Config_detective.qbSettings = {
    enabled = false,
    useNewQBExport = true,
    deadPlayerRefreshTime = 15000,
}


-- Every DOJ + Law Enforcement job on this server (esx_uniquejobs/cad/config_crimescene.lua
-- -> Config_cs.DOJJobs + Config_cs.LawEnforcementJobs). Defined ONCE here and
-- reused everywhere below (final-death blip, department bulletins, CAD's own
-- server-side ALLOWED_CAD_JOBS list in cad/server/main.lua) so there's a
-- single place to edit if you ever add/remove a job from either list.
Config_detective.dojLawJobs = {
    'cid', 'cia', 'marshal', 'fbi', 'judge', 'doa', -- DOJ
    'police', 'sheriff', 'mt',                      -- Law Enforcement
}

-- 'doa' is your dedicated coroner/morgue job (esx_uniquejobs/server/doa_main.lua,
-- own society_doa/society_doj account, and part of Config_detective.dojLawJobs above) —
-- the most fitting job to examine bodies, so it's included here.
Config_detective.whitelist = {
    enabled = true,
    jobs = {
        'police',
        'ambulance',
        'doa',
    }
}

Config_detective.target = {
    enabled = true,
    -- ox_target ships a built-in qtarget compatibility layer
    -- ([BASE]/ox_target/client/compat/qtarget.lua, `provide 'qtarget'`),
    -- so exports['qtarget']:AddTargetEntity(...) is transparently handled
    -- by ox_target. No need to point this at 'ox_target' directly.
    system = 'qtarget'
}

-- Keybinds
Config_detective.keybinds = {
    investigate = 'E'
}


-- Animations used while investigating
Config_detective.animation = {
    enabled = true,
    dict = 'amb@medic@standing@tendtodead@idle_a',
    anim = 'idle_a',
}


-- ============================================================================
-- FORENSICS (fingerprints / ballistics) — only applies to murdered PLAYERS,
-- since the evidence is generated from the real killer at time of death.
-- ============================================================================
Config_detective.forensics = {
    enabled = true,

    -- % chance evidence is left behind when a player is murdered.
    printChance = 35,   -- fingerprint, tied to the killer's character identifier
    casingChance = 60,  -- shell casing, tied to the killer's REAL weapon serial
                         -- (see ESX.GenerateWeaponSerial in essentialmode/server/common.lua —
                         -- the same LAW-/DOJ-/GANG- serials your armories already hand out)

    -- lc-inventory items police/ambulance receive when they collect evidence.
    -- Add these to your `items` table — see sql/kq_detective_forensics.sql.
    printItem = 'evidence_print',
    casingItem = 'evidence_casing',

    -- Usable item that analyzes the most recently collected evidence.
    kitItem = 'kit_azmayeshi',

    -- How long the "Analyzing evidence..." mythic_progbar runs for before
    -- the Kit Azmayeshi reveal fires. Purely dramatic — the result is
    -- already decided the instant the item is used, this just paces it out.
    labAnalysisMs = 5000,
}


-- ============================================================================
-- AUTOPSY MINIGAME — only for jobs in `jobs` (coroner/ambulance), and only
-- when investigating a murdered PLAYER (NPCs skip straight to the notepad).
-- On full success it unlocks the exact shot distance and a precise time of
-- death instead of the rounded "N minutes ago" everyone else sees.
-- ============================================================================
Config_detective.autopsy = {
    enabled = true,
    jobs = { 'doa', 'ambulance' }, -- 'doa' is the real coroner job on this server

    stages = {
        { label = 'Checking vitals...',          duration = 4000 },
        { label = 'Estimating time of death...', duration = 5000 },
        { label = 'Extracting projectile...',    duration = 6000 },
    },
}


-- ============================================================================
-- BODY CLEANUP ("collect body") — 'doa' picks up a dead NPC's corpse for a
-- cash reward, same convention as Config_ambulance.reviveReward in
-- ambulance_main.lua (straight xPlayer.addMoney + an esx_society:logAction
-- entry, so it shows up wherever your existing job logs already go).
--
-- Deliberately NPC-only: this deletes the ped entity, which is never safe to
-- do to a real connected player's character. A downed PLAYER still goes
-- through the normal esx_ambulancejob:revivex flow, not this.
-- ============================================================================
Config_detective.cleanup = {
    enabled = true,
    jobs = { 'doa' },
    reward = 150,
    cooldownMs = 3000,   -- per-officer anti-spam guard on the payout
    bagDuration = 4000,  -- ms the black body bag prop stays on the corpse before it's removed
    bagProp = 'prop_body_bag_01',
}


-- ============================================================================
-- DOJ / CAD / RECORDS INTEGRATION — every piece of this talks to
-- esx_uniquejobs ONLY through its own exported functions (never touches its
-- tables directly, except duckcad_data — see the comment on KQ_InsertCadNote
-- in server/dojintegration.lua for why that one's a narrow direct insert):
--   - server/doj_cases.lua:        CreateExternalCase, AddExternalCharge,
--                                   SetExternalCaseStatus, plus two small
--                                   additions of the same shape this resource
--                                   ships with — AddExternalNote /
--                                   AddExternalSuspect (see the doj_cases.lua
--                                   patch delivered alongside this resource).
--   - server/records_manager.lua:  LogCriminalRecord (already exported)
--   - server/court_docket.lua:     ScheduleExternalHearing (already exported)
--   - server/mugshot_manager.lua:  GetLatestMugshot (already exported)
--   - cad/server/main.lua:         users.WantedLevel (direct SQL, since that
--                                   resource's own DuckMdt:UpdateCharacterStatus
--                                   network event now checks the triggering
--                                   player's job server-side — a real security
--                                   fix on their end, but it means kq_detective
--                                   can no longer fire that event itself; the
--                                   direct UPDATE achieves the exact same
--                                   result their handler does internally)
--
-- Full flow on a real player murder:
--   1. Case opens automatically the instant it happens (no command).
--   2. Evidence collected (print/casing) -> a note is added to that case.
--   3. Kit Azmayeshi runs a short "lab analysis" minigame, then on a match:
--        - suspect added to the DOJ case (dept_case_suspects)
--        - suspect's CAD WantedLevel set + a duckcad_data note logged
--        - a criminal_records entry logged (investigative flag only --
--          no jail time set; sentencing stays a judge's call)
--        - a court hearing auto-scheduled
--        - suspect's mugshot (if one exists) pulled into the Discord log
--        - a department-wide chat bulletin sent to every online DOJ/Law
--          member, not just the collecting officer
--   4. If the victim bleeds out unrevived -> a note is added noting that,
--      alongside the existing green map blip.
-- ============================================================================
Config_detective.dojIntegration = {
    enabled = true,
    casePriority = 'high',        -- 'low' | 'medium' | 'high'
    openedByJob = 'doa',          -- job name shown as the case's opener

    pushCadStatus = true,         -- set WantedLevel + duckcad_data note in Unique_Cad/DuckMdt
    cadWantedLevel = 'wanted',    -- must match one of Config_cs.CadWantedLevels

    logCriminalRecord = true,     -- records_manager.lua LogCriminalRecord (investigative flag, no jail time)
    scheduleHearing = true,       -- court_docket.lua ScheduleExternalHearing
    hearingDelayMinutes = 60,     -- how far out to auto-schedule the hearing
    pullMugshot = true,           -- mugshot_manager.lua GetLatestMugshot, for the Discord log

    broadcastToDept = true,       -- chat bulletin to every online Config_detective.dojLawJobs member
}


-- ============================================================================
-- ACTIVE SCENE ALERT — fires the INSTANT a real player murder happens (not
-- at bleedout), so DOJ/Law can respond to a fresh scene in real time. A red
-- "shots fired" blip + a colored [ Dispatch ] chat line, styled to match
-- server/rob_manager.lua's own robAlert exactly. Separate from
-- Config_detective.finalDeath's green "unattended body" blip below, which
-- only fires if nobody revived the victim in time.
-- ============================================================================
Config_detective.activeScene = {
    blipSprite = 161,       -- GTA's built-in "shooting" blip icon
    blipColor = 1,          -- red
    blipScale = 1.0,
    blipLifetimeMs = 3 * 60000, -- auto-removed after 3 minutes
}


-- ============================================================================
-- FINAL DEATH BLIP — when a downed player bleeds out completely and can no
-- longer be revived, a green body blip is broadcast AUTOMATICALLY (no
-- command needed) to everyone currently in Config_detective.dojLawJobs.
--
-- "Bled out completely" is mirrored from Config_ambulance.BleedoutTimer
-- (esx_uniquejobs/client/config_ambulance.lua = 5 minutes) WITHOUT touching
-- that file: kq_detective already knows the instant someone dies. If that
-- same player is still recorded as dead once the same duration has passed,
-- they were never revived in time — the exact moment ambulance_main.lua's
-- own timer force-respawns them at the hospital.
-- ============================================================================
Config_detective.finalDeath = {
    enabled = true,
    bleedoutMs = 5 * 60000, -- must match Config_ambulance.BleedoutTimer
    notifyJobs = Config_detective.dojLawJobs,
    blipSprite = 280,             -- GTA's built-in "dead body" blip icon
    blipColor = 2,                -- green
    blipScale = 1.0,
    blipLifetimeMs = 10 * 60000,  -- auto-removed after 10 minutes
}


-- ============================================================================
-- DISCORD LOGGING — self-contained (doesn't touch [SCRIPT]/logs), so this is
-- the only place you need to paste a webhook URL for kq_detective's own logs.
-- ============================================================================
Config_detective.discordWebhook = 'WEBHOOK_LINK_HERE'
