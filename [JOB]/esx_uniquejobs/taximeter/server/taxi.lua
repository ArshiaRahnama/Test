...
function setTarget(taxiVeh, targetId)
    local plate = getPlate(taxiVeh)
    currentTaxiDrivers[plate]["target"] = targetId
    currentTaxiDrivers[plate]["vehId"] = taxiVeh
end
...
-- kept for compatibility (used by things not tied to a single charged passenger, e.g. driver-only sync)
...
-- only the driver + whichever single passenger was picked from the F6 menu see the meter
function sendToChargedPlayers(taxiVeh, fnAction)
    local taxi = getTaxi(taxiVeh)
    if not taxi then return end

    if taxi.driveId then
        fnAction(taxi.driveId)
    end
    if taxi.target and taxi.target ~= taxi.driveId then
        fnAction(taxi.target)
    end
end