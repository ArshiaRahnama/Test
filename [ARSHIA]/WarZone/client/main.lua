...
function WarZone(loadHud)
	...
	CreateThread(function()
		...
		-- Fix: the player used to only get warped into the plane AFTER the
		-- ~5-9s flight already happened -- meaning the plane flew off
		-- empty (of the real player) while they stood wherever they
		-- physically were, and TaskWarpPedIntoVehicle was then called on a
		-- vehicle that had already flown far away (likely to fail silently
		-- or place them oddly). This never surfaced before because the
		-- plane was always frozen in place (see the freeze fix above) --
		-- now that it actually flies, the player needs to be aboard before
		...
		-- was still true from the initial setup above, and was only ever
		-- set back to false much later (after the whole flight). A frozen
		-- vehicle can't move for any reason, AI-driven or not, regardless
		-- of what task or speed you give it. Unfreeze it here, once the
		...
		-- task is issued), the pilot's drive task can silently fail to take
		-- hold and the plane just sits there. Check the plane's actual
		-- speed shortly after and re-issue the task (a few times) if it
		-- never picked up, instead of blindly trusting it moved.
		local retries = 0
		while retries < 3 do
			Wait(1200)
			if myEpoch ~= WarZoneEpoch then return end
