------------------------------------------------------------------------------------
-- Vehicle rental - server side - migrated in full from the standalone
-- Unique_Rent resource (that resource has been deleted; everything it did
...
-- Config.RentDurations (multiplier for the chosen duration tier's id), so the
-- client can't lower it by sending an arbitrary amount, model, or duration id.
local function Rent_GetConfigQuote(model, durationId)
	if type(model) ~= "string" then return nil end

	local vehicleCfg = nil
	for _, veh in pairs(Config.RentVehicles) do
	...
	local durationCfg = Config.RentDurations[tonumber(durationId)]
...
AddEventHandler("unique_rent:pay", function(model, durationId)
	...
	local quote = Rent_GetConfigQuote(model, durationId)
	if not quote then
		return
	end
...
ESX.RegisterServerCallback("unique_rent:check", function(source, cb, model, durationId)
	...
	-- SECURITY FIX carried over: this used to fall back to the
	-- client-supplied `amount` whenever the model wasn't found in
	-- Config.RentVehicles -- same class of bug unique_rent:pay used to have.
	-- Now it just refuses to affirm affordability for a model/duration it
	-- doesn't recognize.
	local quote = Rent_GetConfigQuote(model, durationId)
	if not quote then
		cb(false)
...
-- SECURITY FIX carried over: any player could call this with an arbitrary
-- `plate` and delete ANY vehicle on the server (even ones belonging to other
-- players) -- there was no check that the caller actually owns/is using that
-- vehicle. Now it only deletes a vehicle the calling player is currently
-- sitting in as the driver.
-- (Reserved/currently unused: nothing in rent_client.lua actually triggers
-- this event today, same as in the original Unique_Rent - kept in case
-- something else, like an admin tool, needs it.)
...
-- (Rent_Options.have_rented) lives entirely client-side; if their rented
-- vehicle is lost some other way (falls in water, gets blown up, despawns,
-- deleted by an admin, etc.) Rent_ReturnVehicle() can never run because it
-- requires the player to be sitting in that exact vehicle, so they'd
-- otherwise be locked out of renting again until they reconnect. This tells
-- their client to reset that state directly.
