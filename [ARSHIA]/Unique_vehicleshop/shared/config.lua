...
-- (that resource has been deleted; see rent_client.lua / rent_server.lua).
-- Kept as its own Config.Rent* namespace so nothing here can collide with
-- Config.Vehicles / Config.vehicleshop above, which use unrelated shapes.
------------------------------------------------------------------------------------
Config.RentOptions = {
    -- Rentals are tracked with a countdown timer, which is what actually
    -- makes the duration tiers below mean anything.
    ['time'] = true,
...
-- Rental duration tiers, picked from the ox_lib menu at rent time.
-- `multiplier` is applied to each vehicle's base `price` in Config.RentVehicles
-- below (that base price represents the 1.0x / standard tier, i.e. the
-- "1 Hour" one). This must stay a plain sequential array ([1], [2], [3], ...)
-- since the order here is the order the tiers are shown in the menu.
Config.RentDurations = {
...
-- Markers are drawn with ox_lib's `lib.marker`, and the on-foot prompt is
-- ox_lib's `lib.showTextUI` (the "[E] ..." pill in the corner of the screen).
-- `oxType` is any marker name from ox_lib's MarkerType enum.
...
-- `price` below is the BASE price, for the 1 Hour / 1.0x duration tier. The
-- actual charge is base price × the chosen duration's multiplier (see
-- Config.RentDurations above), recomputed server-side. `icon` is a Font
-- Awesome name shown next to the vehicle in the ox_lib menu. `image_name`
-- points at html/rent/<image_name>.png.
Config.RentVehicles = {
    [1] = {
        model = 'neon',
        label = 'Neon',
