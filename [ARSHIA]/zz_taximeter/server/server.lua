...
RegisterNetEvent('zz_taximeter:openTaximeter', function()
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	local veh = GetVehiclePedIsIn(GetPlayerPed(_src), false)
	...
	if isAlreadyTaxi(veh) then
		TriggerClientEvent('esx:showNotification', _src, "Taxi Meter Shoma Baz Ast Lotfan Ebteda An Ra Bebandid !","warning")
	else 
		addTaxi(veh, _src)
		sendAllFromTaxi(veh, function(ply)
			TriggerClientEvent('zz_taximeter:showTaximeter', ply)
...
RegisterNetEvent('zz_taximeter:closeTaximeter', function()
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	local veh = GetVehiclePedIsIn(GetPlayerPed(_src), false)
	...
	if isAlreadyTaxi(veh) then
		...
		TriggerClientEvent('esx:showNotification', _src, "Shoma Taxi Meter Faali Nadarid !","warning")
	end
