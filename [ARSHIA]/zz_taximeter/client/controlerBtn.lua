...
RegisterCommand('opentaximeter', function()

	if canInteractUI(PlayerPedId()) then
		TriggerServerEvent('zz_taximeter:openTaximeter')
	end
	
end, false)

RegisterCommand('closetaximeter', function()

	if canInteractUI(PlayerPedId()) then
		TriggerServerEvent('zz_taximeter:closeTaximeter')
	end
