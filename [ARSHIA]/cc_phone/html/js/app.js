...
$(function() {
    ...
    window.addEventListener("keyup", function(event) {
        if (event.key == 'ß' || event.key == "Escape") {
...
//----------------------------------------------------------------------------------------------------Bank
// EXPANSION: the phone's "Bank" icon has no screen of its own — it hands
// off to the external new_banking resource's own ATM-style UI, exactly the
// way Unique_Phone's bank icon does. The phone closes itself first, then
// asks the client (openBank) to open new_banking on a short delay so the
// phone's own close animation/UI is out of the way first.
$(document).on("click", "#M-App-13", function() {
    closePhone()
    $.post('https://cc_phone/closePhone', JSON.stringify({}))
    $.post('https://cc_phone/openBank', JSON.stringify({}))
})
