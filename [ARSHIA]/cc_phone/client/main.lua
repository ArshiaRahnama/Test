...
-- EXPANSION: Bank — the "Bank" menu icon hands off to the external
-- new_banking resource's own ATM-style UI (exports['new_banking']:OpenBankUI())
-- instead of a built-in bank screen, matching Unique_Phone's integration.
-- The phone JS (app.js) closes the phone itself and posts here; this waits
-- for the phone to be out of the way, then opens new_banking.
RegisterNUICallback('openBank', function(data, cb)
    SetTimeout(300, function()
        if GetResourceState('new_banking') == 'started' then
            ...
            TriggerEvent('cc_phone:sendNotify', 'Bank', 'بانک در دسترس نیست.', 'error', 4000)
        end
    end)
    cb('ok')
end)
...
RegisterNUICallback('joinRadio', function(data, cb)
    ...
    if Config.RadioAccess[frequenz] then
        ...
        if canAccess then
            exports['pma-voice']:addPlayerToRadio(frequenz)
            cb(true)
        else
            cb(false)
        end
...
RegisterNUICallback('leaveRadio', function(data, cb)
    exports['pma-voice']:addPlayerToRadio(0)
end)
...
RegisterNUICallback('getGarage', function(data, cb)
    TriggerServerEvent('chaos_garage:requestVehicles')
    cb({})
end)
...
RegisterCommand("togglephone", function()
    if PhoneData.Locked then
        print("Handy ist gesperrt!")
        return
    end
    
    PhoneData.Open = not PhoneData.Open
    if PhoneData.Open then
        print("Handy geöffnet")
        OpenPhone()
    else
        print("Handy geschlossen")
        ClosePhone()
    end
end, false)

RegisterKeyMapping("togglephone", "Handy öffnen/schließen", "keyboard", "F1")
