...
local function saveXPRank(playerId, xp, rank)
    ...
    local oldRank = PlayerRank[playerId]
    PlayerXP[playerId] = xp
    PlayerRank[playerId] = rank
    ...
    -- Tells essentialmode (server/main.lua) to recompute this player's
    -- inventory capacity (Config.DefaultMaxWeight + Config.WeightPerLevel
    -- per level) now that their rank changed. Only fired when the rank
    -- actually moved, not on every XP tick - GrantXP/RevokeXP call this
    -- for xp changes that don't cross a level boundary too.
    if oldRank ~= rank then
        TriggerEvent('essentialmode:levelChanged', playerId, rank)
    end
end
