...
furni.canPlayerAfford = function(source,price)
  if FurniConfig.UsingESX then
    ...
    if player.money >= price then
      if FurniConfig["UsingESX_V1.2"] then
...
furni.getIdentifier = function(source,ignoreKash)
  if FurniConfig.UsingESX then
    if not ignoreKash and FurniConfig.UsingKashacters then
...
AddEventHandler("furni:playerLogin", function()
  ...
  if FurniConfig.UsingKashacters then
...
SqlReady = function()
  ...
  local dbTable = (FurniConfig and FurniConfig.SaveToTable or "allhousing")
...
if not FurniConfig.ControlSql then
...
if not FurniConfig.ControlAddition then
  furni.placeFurniture = function(source,houseData,itemData,pos,rot,object)
    ...
    if truePrice and furni.canPlayerAfford(source,truePrice) then
      SqlFetch("SELECT * FROM "..(FurniConfig and FurniConfig.SaveToTable or "allhousing"),{},function(retData)
        for _,data in pairs(retData) do
          ...
          if (math.floor(tonumber(houseData.Entry.x)) == math.floor(tonumber(entry.x)) and math.floor(tonumber(houseData.Entry.y)) == math.floor(tonumber(entry.y)) and math.floor(tonumber(houseData.Entry.z)) == math.floor(tonumber(entry.z))) then
            ...
            SqlExecute("UPDATE "..(FurniConfig and FurniConfig.SaveToTable or "allhousing").." SET furniture=@furniture WHERE id=@id",{['@furniture'] = json.encode(jTab),['@id'] = data.id})
...
if not FurniConfig.ControlDeletion then
        ...
        if (math.floor(tonumber(houseData.Entry.x)) == math.floor(tonumber(entry.x)) and math.floor(tonumber(houseData.Entry.y)) == math.floor(tonumber(entry.y)) and math.floor(tonumber(houseData.Entry.z)) == math.floor(tonumber(entry.z))) then
          ...
          if closest and closestDist and closestDist < 1.0 then
            ...
            if truePrice then
              ...
              furni.givePlayerMoney(source,math.floor(truePrice * (FurniConfig.ResaleValue and FurniConfig.ResaleValue/100.0 or 0.5)))
              SqlExecute("UPDATE "..(FurniConfig and FurniConfig.SaveToTable or "allhousing").." SET furniture=@furniture WHERE id=@id",{['@furniture'] = json.encode(jTab),['@id'] = data.id})
              TriggerEvent("Allhousing:SetFurni",houseData,furniture)
            end
          end
...
if not FurniConfig.ControlReplace then
