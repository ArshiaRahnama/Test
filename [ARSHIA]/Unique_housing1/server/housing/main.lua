...
ESX.RegisterServerCallback('Allhousing:CheckOwner', function(source, cb)
	MySQL.Async.fetchAll('SELECT * FROM allhousing WHERE ownername = @ownername', {
		['@ownername'] = GetCharacterName(source)
	}, function(data)
		if data[1] then
			cb(false)
		else
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            local societyAmount = (HousingConfig.CreationJobs[v.ResaleJob].society and math.floor(v.Price * ((100 - v.ResalePercent) / 100)))
            local societyAccount = HousingConfig.CreationJobs[v.ResaleJob].account
...
SellHouse = function(house,price)
  ...
  if v.Entry.x == entry.x and v.Entry.y == entry.y and v.Entry.z == entry.z then
    if v.Owned and v.Owner == GetPlayerIdentifier(_source) then
      if HousingConfig.RemoveFurniture then
        if HousingConfig.RefundFurniture and HousingConfig.RefundPercent then
          TriggerEvent("Allhousing.Furni:GetPrices", function(prices)
            ...
            if v and v.Furniture and type(v.Furniture) == "table" then
              ...
              if count and count > 0 then
                ...
                AddPlayerBank(_source,math.ceil(addVal*(HousingConfig.RefundPercent / 100)))
              end
            end
          end)
        end
      ...
      SqlExecute("UPDATE "..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing").." SET owned=0,price=@price,resalepercent=100,resalejob=@resalejob,furniture=@furniture WHERE id=@id",{['@furniture'] = json.encode(furniTab),['@resalejob'] = "",['@price'] = price,['@id'] = v.Id})
      return
    end
  end
end
...
MortgageHouse = function(house)
  ...
  local entry = house.Entry
  if not v.Owned then
    local price = math.floor((v.Price / 100) * HousingConfig.MortgagePercent)
    local afford
    ...
    if afford then
      ...
      if v.Owner and v.Owner:len() >= 1 then
        ...
        if salePrice > 0 then
          ...
          if v.ResaleJob and v.ResaleJob:len() > 1 then
            local societyAmount = (HousingConfig.CreationJobs[v.ResaleJob].society and math.floor(price * ((100 - v.ResalePercent) / 100)))
            local societyAccount = HousingConfig.CreationJobs[v.ResaleJob].account
            if societyAmount and societyAccount then
              AddSocietyMoney(societyAccount,societyAmount)
            end
...
if HousingConfig.UsingDiscInventory then
  Citizen.CreateThread(function()
    TriggerEvent('disc-inventoryhud:RegisterInventory', {
      name = 'allhousing',
      label = 'Storage',
      slots = (HousingConfig.DiscInventorySlots or 20)
    })
  end)
end

if HousingConfig.ControlCharacters then
  KashChosen = function(charId)
    KashCharacters[source] = charId
  end

  GetHouseData = function(source,entry)
    while not ModReady do Wait(1); end
    local identifier = GetPlayerIdentifier(source,true)
    if HousingConfig.UsingKashacters then
      while not KashCharacters[source] do Wait(1); end
...
RevokeTenancy = function(house)
  ...
  local xPlayer = ESX.GetPlayerFromId(source)
  if xPlayer.permission_level <= 1 then return; end
...
SqlCheck = function()
  ...
  local addToDb = {}
  local curInDb = SqlFetch('SELECT * FROM '..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing"),{})
...
Init = function()
  ...
  while not SqlReady do Wait(1); end
...
BreakLockpick = function()
  TakePlayerItem(source,HousingConfig.LockpickItem)
end

if not HousingConfig.ControlCharacters then
  KashChosen = function(charId)
    KashCharacters[source] = charId
  end
...
UpgradeHouse = function(house,shell)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    ...
    if afford then
      v.Shell = shell
      SyncHouse(v)
      SqlExecute("UPDATE "..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing").." SET shell=@shell WHERE id=@id",{['@shell'] = shell,['@id'] = v.Id})
...
GiveKeys = function(house,target)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    ...
    SqlExecute("UPDATE "..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing").." SET housekeys=@housekeys WHERE id=@id",{['@housekeys'] = json.encode(v.HouseKeys),['@id'] = v.Id})
  end
end
...
SetWardrobe = function(house,wardrobe)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    v.Wardrobe = wardrobe
    SyncHouse(v)
    SqlExecute("UPDATE "..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing").." SET wardrobe=@wardrobe WHERE id=@id",{['@wardrobe'] = json.encode({x = wardrobe.x, y = wardrobe.y, z = wardrobe.z}),['@id'] = v.Id})
...
SetInventory = function(house,inventoryLocation)
  ...
  if v.Owned == true and v.Owner == GetPlayerIdentifier(_source) then
    ...
    SyncHouse(v)
    SqlExecute("UPDATE "..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing").." SET inventorylocation=@inventorylocation WHERE id=@id",{['@inventorylocation'] = json.encode({x = inventoryLocation.x, y = inventoryLocation.y, z = inventoryLocation.z}),['@id'] = v.Id})
...
GetItem = function(house,itemType,itemName,itemCount)
  ...
  if itemType == 'item_money' then
    ...
    --SyncHouse(v)
    SqlExecute("UPDATE "..((HousingConfig and HousingConfig.AllhousingTable) or "allhousing").." SET inventory=@inventory WHERE id=@id",{['@inventory'] = json.encode(v.Inventory),['@id'] = v.Id})
...
AddEventHandler("Allhousing:GetGlobalOffset", function(cb) cb(HousingConfig.SpawnOffset); end)

AddEventHandler("Allhousing:GetHouseById",function(id,callback,source)
  callback(Houses[id],KashCache[source])
end)

if not HousingConfig.OtherStartEvent then
  MySQL.ready(function(...) Citizen.CreateThread(SqlCheck); end)
else
  AddEventHandler(HousingConfig.OtherStartEvent,SqlCheck)
end
