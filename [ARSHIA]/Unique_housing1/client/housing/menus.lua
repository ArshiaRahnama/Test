...
OpenMenu = function(...)
  if HousingConfig.UsingESX then
    if HousingConfig.UsingESXMenu then
      menuType = "ESX"
    elseif HousingConfig.UsingNativeUI then
      menuType = "NativeUI"
    end
  elseif HousingConfig.UsingNativeUI then
    menuType = "NativeUI"
...
UnlockHouse = function(house)
  ...
  if HousingConfig.UsingNativeUI and _Pool then
    _Pool:CloseAllMenus()
  elseif HousingConfig.UsingESX and HousingConfig.UsingESXMenu then
...
SpawnVehicle = function(pos,model,props)  
  ...
  while not NetworkHasControlOfEntity(veh) do NetworkRequestControlOfEntity(veh); Wait(1); end
  while not NetworkGetEntityIsNetworked(veh) do NetworkRegisterEntityAsNetworked(veh); Wait(1); end
...
OpenInventory = function()
  if HousingConfig.UsingESX then
    if not HousingConfig.UsingDiscInventory then
...
SetWardrobe = function(d)  
  ...
  while true do
    if IsControlJustPressed(0,HousingConfig.Controls.Accept) then
      local pos = d.Entry.xyz - GetEntityCoords(GetPlayerPed(-1))
      InsideHouse.Wardrobe = pos + HousingConfig.SpawnOffset
...
SetInventory = function(d)  
  ...
  while true do
    if IsControlJustPressed(0,HousingConfig.Controls.Accept) then
      local pos = d.Entry.xyz - GetEntityCoords(GetPlayerPed(-1))
      InsideHouse.InventoryLocation = pos + HousingConfig.SpawnOffset
...
BuyHouse = function(d)
	ESX.TriggerServerCallback('Allhousing:CheckOwner', function(acces)
		if acces then
			local price = d.Price
			...
			ShowNotification("~r~Shoma Ye Khane Darid!")
		end
	end)
end

MortgageHouse = function(d)
  local price = math.floor((d.Price / 100) * HousingConfig.MortgagePercent)
  if CanPlayerAfford(price) then
...
RaidHouse = function(d)
  EnterHouse(d,not HousingConfig.InventoryRaiding)
end
...
KnockOnDoor = function(d)  
  ...
  while not HasAnimDictLoaded("timetable@jimmy@doorknock@") do RequestAnimDict("timetable@jimmy@doorknock@"); Wait(1); end
  TaskPlayAnim(plyPed, "timetable@jimmy@doorknock@", "knockdoor_idle", 8.0, 8.0, -1, 4, 0, 0, 0, 0 )     
  Wait(1)

  while IsEntityPlayingAnim(plyPed, "timetable@jimmy@doorknock@", "knockdoor_idle", 3) do Citizen.Wait(1); end 
...
BreakInHouse = function(d)  
  ...
  if HousingConfig.LockpickRequired then
  ...
  while not HasAnimDictLoaded("mini@safe_ing") do RequestAnimDict("mini@safe_ing"); Citizen.Wait(1); end
  TaskPlayAnim(plyPed, "mini@safe_ing", "idle_base", 1.0, 1.0, -1, 1, 0, 0, 0, 0 ) 
  Wait(2000)
  if HousingConfig.UsingLockpickV1 then
    TriggerEvent("lockpicking:StartMinigame",4,function(didWin)
      if didWin then
        EnterHouse(d,true)
      else
        ClearPedTasksImmediately(plyPed)
        if HousingConfig.LockpickBreakOnFail then
          TriggerServerEvent("Allhousing:BreakLockpick")
        end
        ShowNotification(Labels["LockpickFailed"])
        FreezeEntityPosition(plyPed,false)
        for k,v in pairs(HousingConfig.PoliceJobs) do
          TriggerServerEvent("Allhousing:NotifyJobs",k,string.format(Labels["NotifyRobbery"],zoneName),d.Entry)
          if HousingConfig.UsingInteractSound then
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 20.0, 'security-alarm', 1.0)
          end
        end
      end
    end)
  elseif HousingConfig.UsingLockpickV2 then
    ...
    if HousingConfig.UsingProgressBars then
      exports["progressBars"]:startUI(HousingConfig.LockpickTime * 1000,Labels["ProgressLockpicking"])
    end
    Wait(HousingConfig.LockpickTime * 1000)
    if math.random(100) < HousingConfig.LockpickFailChance then
      if HousingConfig.LockpickBreakOnFail then
        TriggerServerEvent("Allhousing:BreakLockpick")
      end
      for k,v in pairs(HousingConfig.PoliceJobs) do
...
SpawnHouse = function(d)
  ...
  RequestModel(hash)
  while not HasModelLoaded(hash) and GetGameTimer() - start < 30000 do Wait(1); end
  if not HasModelLoaded(hash) then
    ShowNotification(string.format(Labels["InvalidShell"],model))
    return false,false
  end

  local shell = CreateObject(hash, d.Entry.x + HousingConfig.SpawnOffset.x,d.Entry.y + HousingConfig.SpawnOffset.y,d.Entry.z - 30.0 + HousingConfig.SpawnOffset.z,false,false)
  FreezeEntityPosition(shell,true)

  start = GetGameTimer()
  while not DoesEntityExist(shell) and GetGameTimer() - start < 30000 do Wait(1); end
  ...
  if ShellExtras[d.Shell] then
    for objHash,data in pairs(ShellExtras[d.Shell]) do
      ...
      start = GetGameTimer()
      while not HasModelLoaded(objHash) and GetGameTimer() - start < 10000 do Wait(1); end
      if HasModelLoaded(objHash) then
        local pos = d.Entry.xyz + data.offset + HousingConfig.SpawnOffset
...
TeleportInside = function(d,v)  
  local exitOffset = vector4(ShellOffsets[d.Shell]["exit"].x - HousingConfig.SpawnOffset.x,ShellOffsets[d.Shell]["exit"].y - HousingConfig.SpawnOffset.y,ShellOffsets[d.Shell]["exit"].z - HousingConfig.SpawnOffset.z,ShellOffsets[d.Shell]["exit"].w)
  ...
  if shell and extras then
    ...
    while (not HasCollisionLoadedAroundEntity(plyPed) and GetGameTimer() - start_time < 5000) do Wait(1); end
    FreezeEntityPosition(plyPed,false)
...
SellHouse = function(d)
  ...
  if not d.MortgageOwed or d.MortgageOwed <= 0 then
    Open(Labels["SetSalePrice"],(HousingConfig.UsingESX and HousingConfig.UsingESXMenu and "ESX" or "Native"), function(data)
      local price = (tonumber(data) and tonumber(data) > 0 and tonumber(data) or 0)
      local floored = math.max(1,math.floor(tonumber(price)))

      Wait(100)

      if HousingConfig.UsingESX and HousingConfig.UsingESXMenu then
...
RepayMortgage = function(d)
  ...
  local min_repayment = math.floor((d.MortgageOwed / 100) * HousingConfig.MortgageMinRepayment)
  if GetPlayerCash() > min_repayment or GetPlayerBank() > min_repayment then
    Open(string.format("Min: $%i",min_repayment),(HousingConfig.UsingESX and HousingConfig.UsingESXMenu and "ESX" or "Native"),function(res)
