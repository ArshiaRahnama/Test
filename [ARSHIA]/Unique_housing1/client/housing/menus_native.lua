...
NativeGarageOwnedMenu = function(d)
  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    if not HousingConfig.GarageTheft then return; end
    ...
    elseif HousingConfig.UsingLockpickV2 then
      ...
      if HousingConfig.UsingProgressBars then
        exports["progressBars"]:startUI(HousingConfig.LockpickTime * 1000,"Lockpicking Door")
      end
...
NativeExitEmptyMenu = function(d)
  ...
  if (d.Owned) then
    ...
    if HousingConfig.PoliceCanRaid and HousingConfig.PoliceJobs[jobName] then 
      if GetPlayerJobRank() >= HousingConfig.PoliceJobs[jobName].minRank then
        if d.Unlocked then
          local lock = NativeUI.CreateItem("Lock House",d.Shell)
          lock.Activated = function(...) LockHouse(d); end
          exitMenu:AddItem(lock)
...
NativeEntryOwnedMenu = function(d)
  ...
  if not hasKeys then
    ...
    if HousingConfig.CreationJobs and HousingConfig.CreationJobs[jobName] then
      if GetPlayerJobRank() >= HousingConfig.CreationJobs[jobName].minRank then
        NativeCreateMortgageMenu(d,entryMenu)
      end
    end

    if d.Shell then
      if HousingConfig.HouseTheft and not isPolice then
...
NativeEntryEmptyMenu = function(d)
  ...
  mortgage:RightLabel("$"..math.floor(d.Price / 100) * HousingConfig.MortgagePercent)
