...
RegisterCommand("createhouse", function(...)
    ...
    ESX.TriggerServerCallback("esx_aduty:getAdminPerm", function(aperm)
	  if aperm <= 1 then
		return ESX.ShowNotification("You Dont Have Permission")
	  end
	end)

    ShowNotification(Labels["AcceptDrawText"]..Labels["SetEntry"])

    while not IsControlJustPressed(0,HousingConfig.Controls.Accept)  do Wait(1); end
    while     IsControlPressed(0,HousingConfig.Controls.Accept)      do Wait(1); end
    ...
    while not IsControlJustPressed(0,HousingConfig.Controls.Accept) and not IsControlJustPressed(0,HousingConfig.Controls.Cancel) do Wait(1); end    
    while     IsControlPressed(0,HousingConfig.Controls.Accept)          or IsControlPressed(0,HousingConfig.Controls.Cancel)     do Wait(1); end

    local garageLocation = false
    if IsControlJustReleased(0,HousingConfig.Controls.Accept) then
    ...
    Open("Set Sale Price",(HousingConfig.UsingESX and HousingConfig.UsingESXMenu and "ESX" or "Native"),function(data)
        ...
        if HousingConfig.UsingESX and HousingConfig.UsingESXMenu then
          ...
          interior.Activated = function(...)
            ...
            while true do
              ...
              local int = GetInteriorAtCoords(pos.x,pos.y,pos.z)
              ShowHelpNotification(Labels['Interior']..": "..int)
              if IsControlPressed(0,HousingConfig.Controls.Accept) then
                if int and int >= 1 then
                  is_interior = int
                  ShowNotification(Labels['Interior']..": "..int)
                  return
      ...
      while not is_interior and not is_shell do Wait(1); end
      ...
      if is_shell then
        ...
        if HousingConfig.UsingESX and HousingConfig.UsingESXMenu then
          ...
          ESX.UI.Menu.Open('default', GetCurrentResourceName(), "create_house_shell",{
              title    = "Are you sure about building a house?",
              align    = 'center',
              elements = elements,
            }, 
            function(submitData,submitMenu)
        ...
        if HousingConfig.UseDoors then
          Wait(500)
