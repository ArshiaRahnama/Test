...
function GetBlipData(t,zoneCoord)
  if t == "owner" then
    if not HousingConfig.HideOwnBlips then
      local color,sprite
      if HousingConfig.UseZoneSprites then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        sprite = (HousingConfig.ZoneBlipSprites[zone] and HousingConfig.ZoneBlipSprites[zone].OwnerSprite or HousingConfig.BlipOwnerSprite)
      else
        sprite = HousingConfig.BlipOwnerSprite
      end
      if HousingConfig.UseZoneColoring then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        color = (HousingConfig.ZoneBlipColors[zone] and HousingConfig.ZoneBlipColors[zone].OwnerColor or HousingConfig.BlipOwnerColor)
      else
        color = HousingConfig.BlipOwnerColor
      end
      return color,sprite
    else
      return false
  ...
  elseif t == "owned" then
    if not HousingConfig.HideSoldBlips then
      local color,sprite
      if HousingConfig.UseZoneSprites then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        sprite = (HousingConfig.ZoneBlipSprites[zone] and HousingConfig.ZoneBlipSprites[zone].OwnedSprite or HousingConfig.BlipOwnedSprite)
      else
        sprite = HousingConfig.BlipOwnedSprite
      end
      if HousingConfig.UseZoneColoring then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        color = (HousingConfig.ZoneBlipColors[zone] and HousingConfig.ZoneBlipColors[zone].OwnedColor or HousingConfig.BlipOwnedColor)
      else
        color = HousingConfig.BlipOwnedColor
      end
      return color,sprite
    else
      return false
  ...
  elseif t == "empty" then
    if not HousingConfig.HideEmptyBlips then
      local color,sprite
      if HousingConfig.UseZoneSprites then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        sprite = (HousingConfig.ZoneBlipSprites[zone] and HousingConfig.ZoneBlipSprites[zone].EmptySprite or HousingConfig.BlipEmptySprite)
      else
        sprite = HousingConfig.BlipEmptySprite
      end
      if HousingConfig.UseZoneColoring then
        local zone = GetZoneAtCoords(zoneCoord.x,zoneCoord.y,zoneCoord.z)
        color = (HousingConfig.ZoneBlipColors[zone] and HousingConfig.ZoneBlipColors[zone].EmptyColor or HousingConfig.BlipEmptyColor)
      else
        color = HousingConfig.BlipEmptyColor
      end
      return color,sprite
    else
      return false
...
function Callback(event,...)
  ...
  while not (_Callbacks[myId]) and (GetGameTimer() - start < 30000) do Wait(1); end
...
SetWeatherAndTime = function(syncTime)
  if not syncTime then
    if HousingConfig.UsingVSync then
