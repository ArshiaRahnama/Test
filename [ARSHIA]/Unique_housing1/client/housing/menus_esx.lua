...
ESXExitOwnerMenu = function(d)
  local elements = {}
  if HousingConfig.UseHouseInventory then
    if HousingConfig.AllowHouseSales then
      if d.Shell then
        elements = {
...
ESXExitOwnedMenu = function(d)
  ...
  elseif d.MortgageOwed and d.MortgageOwed > 0 then
    ...
    if HousingConfig.CreationJobs[job] then
      if GetPlayerJobRank() >= HousingConfig.CreationJobs[job].minRank then
...
ESXExitEmptyMenu = function(d)
  ...
  if d.Owned and d.Shell then
    ...
    if HousingConfig.PoliceCanRaid and certifiedPolice then
      if d.Unlocked then
...
ESXGarageOwnerMenu = function(d)
  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    if ownerInfo.owned and ownerInfo.owner then
      canStore = true
    elseif ownerInfo.owned and HousingConfig.StoreStolenPlayerVehicles then
      canStore = true
    else
      canStore = false
    end
...
ESXGarageOwnedMenu = function(d)
  ...
  if IsPedInAnyVehicle(plyPed,false) then
    ...
    if not HousingConfig.GarageTheft then 
      FreezeEntityPosition(GetPlayerPed(-1),false)
      return
    end
    ...
    while not HasAnimDictLoaded("mini@safe_cracking") do RequestAnimDict("mini@safe_cracking"); Citizen.Wait(1); end
...
ESXEntryOwnerMenu = function(d)
  local elements
  if HousingConfig.AllowHouseSales then
    if d.Garage and HousingConfig.AllowGarageMovement then
      if d.Shell then
        elements = {
...
ESXEntryOwnedMenu = function(d)
  ...
  if d.Shell then
    ...
    elseif certifiedPolice then
      ...
      if HousingConfig.PoliceCanRaid then
        table.insert(elements,{label = Labels['RaidHouse'],value = "Raid"})
      end
    else
      table.insert(elements,{label = Labels['KnockHouse'],value = "Knock"})
      if HousingConfig.HouseTheft then
...
ESXEntryEmptyMenu = function(d)
  local elements = {}
  if d.ResaleJob and d.ResaleJob:len() > 0 and HousingConfig.AllowMortgage then
    if d.Shell then
      elements = {
        [1] = {label = Labels['Buy'].." [$"..d.Price.."]", value = "Buy"},
        [2] = {label = Labels['Mortgage'].." [$"..math.floor((d.Price / 100)*HousingConfig.MortgagePercent).."]", value='Mortgage'},
