...
RegisterNUICallback("PutIntoTrunk", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        ...
        if data.item.type == "item_weapon" then
            ...
            if not HasPedGotWeapon(playerPed, weaponHash, false) then return end
            count = GetAmmoInPedWeapon(playerPed, weaponHash)
        end
    ...
    TaskPlayAnim(playerPed, dict, "purchase_beerbox_shopkeeper", 8.0, 2.0, -1, 48, 2, 0, 0, 0 )
...
function refreshBoxInventory()
    ESX.TriggerServerCallback("Mani_Code:OpenLootMenu", function(data)
...
RegisterNUICallback("TakeFromBox", function(data, cb)
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        TriggerServerEvent("Mani_Code:RefreshInventory", data.item.type, data.item.name, tonumber(data.number), gangLoot)
    end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        elseif data.item.name == "carradio" then
            closeInventory()
            TriggerServerEvent("esx:useItem", data.item.name)
        else
