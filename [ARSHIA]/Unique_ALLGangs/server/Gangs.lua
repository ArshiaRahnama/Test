...
ESX.RegisterServerCallback('FMGangs:EditItemAccess', function(source, cb, GangName, GradeNumber, itemName, value)
    ...
    MySQL.Async.execute('UPDATE gang_grades SET access = @access WHERE gang_name = @gang_name AND grade = @grade',
    {
        ...
        -- itself always reads live data, but esx_inventory only
        ...
        for key, _ in pairs(Gangs[GangName].armory or {}) do
            ...
            pcall(function() exports['esx_inventory']:refreshStashViewers(stashId) end)
        end
        cb(value)
    end)
end)
...
-- essentialmode<->esx_inventory callback bug was: a two-sided
...
-- exact same problem: esx_inventory's own registerStashAccessCheck
...
-- this resource (never crosses any boundary). esx_inventory is only
-- told the stash id + which resource owns it
-- (GetInvokingResource() on esx_inventory's side), and relays actual
...
EnsureArmoryStash = function(playergang, key, armory)
    ...
    -- Unique_ALLGangs' own memory - if esx_inventory gets restarted on
