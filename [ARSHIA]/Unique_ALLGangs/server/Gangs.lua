...
-------------------------------------------------------------------
-- Per-vehicle-model access, same pattern as EditItemAccess above -
-- nested inside the same `access` blob (access.vehicleAccess), no DB
-- schema change needed. Enforced again in FMGangs:RegisterGangVehicle
-- (server/boss.lua) so it can't be bypassed by calling that event
-- directly with a blocked model.
-------------------------------------------------------------------
    ...
    if items then
        for _, slot in pairs(items) do
            if slot and slot.name and not seen[slot.name] then
                seen[slot.name] = true
                -- FEATURE (requested: make item access "cooler" -
                -- show real names instead of raw item ids like
                -- WEAPON_PISTOL): exports.ox_inventory:Items(name)
                -- returns the item's real display label, same export
                -- ox_inventory's own crafting-integration docs
                -- recommend for exactly this.
                local itemDef = exports.ox_inventory:Items(slot.name)
                table.insert(names, { name = slot.name, label = (itemDef and itemDef.label) or slot.name })
            end
        end
    end
