...
RegisterServerEvent('Unique_AdminPanel:DutyLogToggle')
AddEventHandler('Unique_AdminPanel:DutyLogToggle', function(isOnDuty)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
...
RegisterServerEvent('Unique_AdminPanel:SetFlag')
AddEventHandler('Unique_AdminPanel:SetFlag', function(targetId, note)
    local source = source
...
RegisterServerEvent('Unique_AdminPanel:ClearFlag')
AddEventHandler('Unique_AdminPanel:ClearFlag', function(targetId)
    local source = source
    if not IsOnDutyAdmin(source) then return end
    local Target = ESX.GetPlayerFromId(tonumber(targetId))
    if not Target then return end
