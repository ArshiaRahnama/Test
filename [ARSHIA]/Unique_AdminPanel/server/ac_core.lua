...
CreateThread(function()
    ...
    local ok, err = pcall(function()
        local resourceName = GetCurrentResourceName()
        -- was "configs/fire-config.lua" before UNIQUE_AC was merged into
        -- Unique_AdminPanel; that file is now shared/ac_config.lua
        local current = LoadResourceFile(resourceName, "shared/ac_config.lua")
        if not current then return end
        local currentHash = uniqueacChecksum(current)
        local lastHash = LoadResourceFile(resourceName, "shared/backups/.last-hash")
        ...
        local indexRaw = LoadResourceFile(resourceName, "shared/backups/.index") or ""
        local names = {}
        for line in indexRaw:gmatch("[^\r\n]+") do names[#names + 1] = line end
        names[#names + 1] = ("ac_config-%s.lua"):format(stamp)

        local keep = math.max(1, tonumber(UNIQUE_AC.ConfigBackup.Keep) or 20)
        while #names > keep do
            local oldest = table.remove(names, 1)
            SaveResourceFile(resourceName, "shared/backups/" .. oldest, "", -1)
        end
        SaveResourceFile(resourceName, "shared/backups/.index", table.concat(names, "\n"), -1)
    end)
...
function StartAntiCheat()
    ...
    -- paths updated after UNIQUE_AC was merged into Unique_AdminPanel:
    -- tables/fire-*.lua -> shared/tables/fire-*.lua, configs/fire-config.lua
    -- -> shared/ac_config.lua (this is what was printing "Some Files Of
    -- UNIQUE_AC Not Found!" on every boot - the check was still looking at
    -- the old, pre-merge paths)
    local resources = {
        "shared/ac_config.lua", "shared/tables/fire-event.lua", "shared/tables/fire-explosions.lua",
        "shared/tables/fire-name.lua", "shared/tables/fire-object.lua", "shared/tables/fire-peds.lua",
        "shared/tables/fire-plate.lua", "shared/tables/fire-vehicle.lua", "shared/tables/fire-weapon.lua",
        "shared/tables/fire-words.lua", "shared/tables/fire-task.lua", "shared/tables/fire-anim.lua",
        "shared/tables/fire-emoji.lua"
    }
...
-- Exported so other resources (esx_uniquejobs's job files call this on
-- confirmed cheat attempts) can actually reach it. It existed as a plain
-- global only, so exports.Unique_AdminPanel:BanPlayer(...) from another
-- resource has always thrown "no such export" if it ever got called --
-- see the exports.UNIQUE_AC guard fix across esx_uniquejobs's job files
...
-- MISSING EXPORT FIX: same issue as BanPlayer above - this one got missed
