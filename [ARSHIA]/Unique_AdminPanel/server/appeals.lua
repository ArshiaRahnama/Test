...
-- goes through the real UnbanPlayer function (server/ac_core.lua, merged
...
RegisterServerEvent('Unique_AdminPanel:ReviewAppeal')
AddEventHandler('Unique_AdminPanel:ReviewAppeal', function(appealId, approve)
    local source = source
    ...
    MySQL.Async.fetchAll(
        ...
        function(rows)
            ...
            if approve and row.ban_id and type(UnbanPlayer) == 'function' then
                UnbanPlayer(row.ban_id, ("Admin %s (%s) via appeal #%s"):format(adminName, source, appealId))
            end
