...
-- ExemptFromAntiCheat is defined once, globally, in server/admin_tools.lua
-- (previously duplicated here as a separate local copy - now that
-- UNIQUE_AC is merged into this same resource, both call sites share the
-- one real implementation instead of two copies of the same wrapper).
...
-- site here uses 5000ms, so clamp to that range and rate-limit.
...
TriggerEvent('es:addAdminCommand', 'changeworld', 1, function(source, args)
	...
	local WebHook = GetConvar('unique_commands_1_changeworld_webhook', '')
	local xPlayer = ESX.GetPlayerFromId(source)
	if args[1] then
		if args[2] then
...
TriggerEvent('es:addAdminCommand', 'st', 2, function(source, args, user)
	if args[1] and args[2] then
		if(tonumber(args[1]) and (tostring(args[2])) and GetPlayerName(tonumber(args[1])))then
			...
			for i,v in ipairs(AdutyConfig.Locations) do
            ...
            if tonumber(args[1]) and args[2] and tonumber(args[3]) then
                ...
                if xPlayer then
                    if ESX.DoesJobExist(args[2], tonumber(args[3])) then
                        ...
                        local webhook = GetConvar('unique_commands_1_webhook', '')
                        local message = {
                            embeds = {{
                                title = "SetJob Log",
            ...
            if tonumber(args[1]) and args[2] and tonumber(args[3]) then
                ...
                if xPlayer then
                    if ESX.DoesGangExist(args[2], tonumber(args[3])) then
                        ...
                        local webhook = GetConvar('unique_commands_1_webhook_2', '')
                        local message = {
                            embeds = {{
                                title = "SetGang Log",
                                description = string.format(
            ...
            if tonumber(args[1]) and args[2] then
                ...
                if count ~= nil then
                    if xPlayer.getInventoryItem(item) ~= nil then
                        ...
                        TrackEconomyCommand(source, "giveitem", ("target: %s | item: %s x%s"):format(xPlayer.name, item, count))
    ...
    function(source, args, user)
        ...
        if xPlayer.get("aduty") then
            if tonumber(args[1]) and args[2] then
                ...
                -- "WEAPON_WEAPON_SMG" item that ESX.Items/AdutyConfig.Weapons
    ...
    function(source, args, user)
        if args[1] then
            ...
            if type(targetId) == "number" then
                ...
                local webhook = GetConvar('unique_commands_1_webhook_3', '')
                local message = {
                    embeds = {{
            ...
            if tonumber(args[1]) and tonumber(args[2]) and GetPlayerName(tonumber(args[1])) then
                ...
                if xPlayer.permission_level < 12 then
                    ...
                    TriggerEvent(
                        ...
                        source,
                        info,
                        GetConvar('unique_commands_1_wh1', '')
                    )
                    TriggerClientEvent(
                        "esx:showNotification",
                        source,
...
RegisterCommand("aa", function(source)
    ...
    if xPlayer.permission_level >= 7 then
        ...
        local webhook = GetConvar('unique_commands_1_webhook_4', '')
        local message = {
            embeds = { {
...
-- RENAMED from "ajailoffline" to "ajailoffline_legacy": Unique_Punishment
-- (now merged in as server/punish_jail.lua) has its OWN /ajailoffline that
-- writes a richer sentence shape (reason, proper unjail threshold,
-- startedAt) to the exact same `users`.`jail` column - and its own client
-- UI/countdown only understands that richer shape. Both commands existed
-- under the identical name even before this merge (esx_aduty and
-- Unique_Punishment were separate resources with a global RegisterCommand
-- namespace), so depending on server.cfg start order, one of them was
-- already silently shadowing the other. Unique_Punishment's version is
-- kept as the canonical /ajailoffline; this simpler one is kept available
-- under a distinct name rather than deleted outright.
RegisterCommand(
    "ajailoffline_legacy",
    function(source, args, users)
        local xPlayer = ESX.GetPlayerFromId(source)
