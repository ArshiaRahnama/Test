...
-- Real underlying function (from server/ac_core.lua, migrated from
-- UNIQUE_AC's fire-server.lua) - grants a real, time-boxed anti-cheat
-- exemption. This used to go through exports['UNIQUE_AC']:ExemptPlayer
-- when UNIQUE_AC was a separate resource; now that it's merged into this
-- same resource, call the real function directly (no export indirection
-- needed). The `if` guard is defensive only, in case ac_core.lua somehow
-- failed to load.
function ExemptFromAntiCheat(targetId, ms, kinds)
    if type(UNIQUE_AC_CHANGE_TEMP_WHHITELIST) ~= 'function' then return end
    pcall(UNIQUE_AC_CHANGE_TEMP_WHHITELIST, targetId, true, ms or 5000)
end
...
AddEventHandler('Unique_AdminPanel:FreezePlayer', function(targetId)
    ...
    TriggerClientEvent('Unique_AdminPanel:ApplyFreeze', targetId, FrozenPlayers[targetId])
...
RegisterServerEvent('Unique_AdminPanel:HealPlayer')
AddEventHandler('Unique_AdminPanel:HealPlayer', function(targetId)
    local source = source
    if not IsOnDutyAdmin(source) then return end
    targetId = tonumber(targetId)
    if not targetId or not ESX.GetPlayerFromId(targetId) then return end

    TriggerClientEvent('Unique_AdminPanel:ApplyHeal', targetId)
...
RegisterServerEvent('Unique_AdminPanel:RevivePlayer')
AddEventHandler('Unique_AdminPanel:RevivePlayer', function(targetId)
    local source = source
    if not IsOnDutyAdmin(source) then return end
    targetId = tonumber(targetId)
    if not targetId or not ESX.GetPlayerFromId(targetId) then return end

    TriggerClientEvent('Unique_AdminPanel:ApplyRevive', targetId)
...
RegisterServerEvent('Unique_AdminPanel:SetButtonPerm')
AddEventHandler('Unique_AdminPanel:SetButtonPerm', function(buttonId, minLevel)
    local source = source
...
RegisterCommand('akick', function(source, args)
    ...
    TriggerClientEvent('Unique_AdminPanel:PlayKickScene', targetId)
...
RegisterCommand('aban', function(source, args)
    ...
    SetTimeout(12000, function()
        ...
        if permanent then
            ...
            -- uniqueac_banlist. UNIQUE_AC used to be a separate resource
            -- (hence going through exports.UNIQUE_AC:BanPlayer); it's now
            -- merged into this same resource, so BanPlayer (from
            -- server/ac_core.lua) is called directly.
            if type(BanPlayer) ~= 'function' then
                TriggerClientEvent('esx:showNotification', source, "~r~BanPlayer unavailable - cannot issue an enforced permanent ban.")
                return
            end
            local ok, banId = BanPlayer(targetId, reason, issuer)
            if not ok then
...
RegisterCommand('aunban', function(source, args)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `unique_adminmenu_bans` WHERE `id` = @id AND `active` = 1", { ['@id'] = recordId }, function(rows)
        ...
        if row.external_ban_id and type(UnbanPlayer) == 'function' then
            UnbanPlayer(row.external_ban_id, ("Admin %s (%s)"):format(GetPlayerName(source), source))
        end
...
-- BanPlayer (server/ac_core.lua) is only called for the permanent case above).
...
RegisterServerEvent('Unique_AdminPanel:AddNote')
AddEventHandler('Unique_AdminPanel:AddNote', function(targetId, note)
    local source = source
    if not IsOnDutyAdmin(source) then return end
    targetId = tonumber(targetId)
    local Target = ESX.GetPlayerFromId(targetId)
...
RegisterServerEvent('Unique_AdminPanel:SpawnVehicle')
AddEventHandler('Unique_AdminPanel:SpawnVehicle', function(model, plate)
    local source = source
    ...
    TriggerClientEvent('Unique_AdminPanel:ApplySpawnVehicle', source, model, plate)
...
-- record at all.
RegisterServerEvent('Unique_AdminPanel:ImpoundRecorded')
AddEventHandler('Unique_AdminPanel:ImpoundRecorded', function(plate, modelLabel, reason, adminName)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('Unique_AdminPanel:ReleaseImpound')
AddEventHandler('Unique_AdminPanel:ReleaseImpound', function(recordId)
    local source = source
    if not IsOnDutyAdminFor(source, 'btn_impound') then DenyButtonAccess(source, 'btn_impound') return end
...
AddEventHandler('Unique_AdminPanel:BringTarget', function(targetId)
    ...
    TriggerClientEvent('Unique_AdminPanel:ApplyTeleportCoords', targetId, coords.x, coords.y, coords.z)
...
RegisterServerEvent('Unique_AdminPanel:SetWeather')
AddEventHandler('Unique_AdminPanel:SetWeather', function(weatherName)
    local source = source
    ...
    TriggerClientEvent('Unique_AdminPanel:ApplyWeather', -1, weatherName)
    LogAdminAction(source, "set-weather", weatherName)
end)

RegisterServerEvent('Unique_AdminPanel:SetTime')
AddEventHandler('Unique_AdminPanel:SetTime', function(hour, minute)
    local source = source
    ...
    TriggerClientEvent('Unique_AdminPanel:ApplyTime', -1, hour, minute)
    LogAdminAction(source, "set-time", ("%02d:%02d"):format(hour, minute))
end)

RegisterServerEvent('Unique_AdminPanel:TeleportCoords')
AddEventHandler('Unique_AdminPanel:TeleportCoords', function(x, y, z)
    local source = source
    if not IsOnDutyAdmin(source) then return end
    x, y, z = tonumber(x), tonumber(y), tonumber(z)
    if not (x and y and z) then return end
    ExemptFromAntiCheat(source, 5000, { teleport = true, speed = true })
    TriggerClientEvent('Unique_AdminPanel:ApplyTeleportCoords', source, x, y, z)
    LogAdminAction(source, "teleport-coords", ("%.2f, %.2f, %.2f"):format(x, y, z))
end)

RegisterServerEvent('Unique_AdminPanel:AntiCheatExempt')
...
RegisterServerEvent('Unique_AdminPanel:SaveLocation')
AddEventHandler('Unique_AdminPanel:SaveLocation', function(name, x, y, z)
    local source = source
    if not IsOnDutyAdmin(source) then return end
