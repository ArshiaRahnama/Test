...
-- PunishConfig.ServiceLocation. Populated when a sentence starts/resumes, cleared
...
-- distinction for the exact same class of action. PunishConfig.AdminPermissionLevel
...
local function IsAllowedToSentence(xSender)
	...
	local minLevel = PunishConfig.AdminPermissionLevel or 2
...
function releaseFromCommunityService(target)
	...
	-- PunishConfig.ReleaseLocation. If Unique_Punishment:CS_ReleaseAck hasn't come
...
-- apply the existing PunishConfig.ServiceExtensionOnEscape penalty and log it.
...
local function PenalizeEscape(targetSource, identifier, detectedBy)
	...
	MySQL.Async.execute('UPDATE communityservice SET actions_remaining = actions_remaining + @extension_value WHERE identifier = @identifier', {
		['@identifier'] = identifier,
		['@extension_value'] = PunishConfig.ServiceExtensionOnEscape,
	})
	TriggerClientEvent('esx:showNotification', targetSource, ("~r~Escape attempt detected - +%s actions added to your sentence."):format(PunishConfig.ServiceExtensionOnEscape))
	if LogAdminAction then
