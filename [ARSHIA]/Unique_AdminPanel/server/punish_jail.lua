...
local function ExemptFromAntiCheat(targetId, ms, kinds)
	if GetResourceState('Unique_AdminPanel') ~= 'started' then return end
	pcall(function()
		exports['Unique_AdminPanel']:ExemptPlayer(targetId, ms or 5000, kinds)
	end)
end
...
local function DecodeJailData(raw, identifier)
	...
	return {
		...
		unjail = data.unjail or PunishConfig.AdminJail.unjail,
...
AddEventHandler('arshia_jail:sendto',function (target, type, time, reason, unjail)
	...
	if type == 'admin' then
		if not xPlayer.permission_level or xPlayer.permission_level < (PunishConfig.AdminPermissionLevel or 2) then
			if exports.Unique_AdminPanel then
				exports.Unique_AdminPanel:BanPlayer(source, 'Cheat Lua Executer', 'Tried arshia_jail:sendto (admin) without permission')
			end
			return
		end
	elseif type == 'faction' then
		if not IsJobAllowed(xPlayer.job.name) then
			if exports.Unique_AdminPanel then
				exports.Unique_AdminPanel:BanPlayer(source, 'Cheat Lua Executer', 'Tried arshia_jail:sendto (faction) without an allowed job')
			end
			return
		end
	else
...
AddEventHandler("arshia_jail:UnjailPlayer", function(id)
	...
	if not zPlayer or not zPlayer.permission_level or zPlayer.permission_level < 2 then
		if exports.Unique_AdminPanel then
			exports.Unique_AdminPanel:BanPlayer(source, 'Cheat Lua Executer', 'Tried arshia_jail:UnjailPlayer without permission')
		end
		return
	end
...
TriggerEvent('es:addAdminCommand', 'ajail', 2, function(source, args, user)
	...
	TriggerClientEvent("arshia_jail:JailPlayer", source, target, 'admin', time, reason, PunishConfig.AdminJail.unjail)
