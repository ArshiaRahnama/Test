		...
		if xPlayer.get('aduty') then
			...
			if reports[args[1]] then
				...
				if reports[args[1]].status == "pending" or xPlayer.permission_level >= 9 then
					...
					-- EXPANSION: lets Unique_AdminPanel ask the reporter for a
					...
					-- esx_Report:CreateReport) lets Unique_AdminPanel compute
					-- a per-admin average response time.
					TriggerEvent('Unique_AdminPanel:ReportClosed', report.owner.id, args[1], closer, report.time)
