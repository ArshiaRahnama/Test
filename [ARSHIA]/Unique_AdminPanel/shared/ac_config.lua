...
UNIQUE_AC.KnownConflicts = {
    Enable = true,
    Resources = {
        -- "esx_aduty" removed: that resource was merged into Unique_AdminMenu
        -- (server/aduty_*.lua / client/aduty_*.lua in that resource) and no
        -- longer runs as its own separate resource, so this entry could
        -- never match anything anymore. If a standalone "esx_aduty" is ever
        -- reintroduced (e.g. a different/unofficial copy), add it back.
    }
}
...
UNIQUE_AC.Integrity = {
    ...
    Files        = {
        ...
        -- (was src/fire-server.lua, src/fire-client.lua, src/fire-menu.lua,
