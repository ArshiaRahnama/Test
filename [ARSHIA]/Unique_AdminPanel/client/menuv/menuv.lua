----------------------- [ MenuV ] -----------------------
-- MenuV by ThymonA (GPL-3.0) - https://github.com/ThymonA/menuv
-- Embedded into Unique_AdminPanel: the library, the old standalone `menuv`
-- resource's glue code (NUI callbacks, key bindings, hide-on-pause) and its
-- NUI page (html/menuv) are merged, so no separate `menuv` resource is needed.
-- Changes vs upstream: no exports, own private _ENV (only `MenuV` is global),
-- NUI messages tagged `_menuv`, unique key-command names, idle-friendly threads.
----------------------- [ MenuV ] -----------------------
...
---@param input any
local MenuV_Hidden = false
local SEND_NUI_MESSAGE = function(input)
    if (type(input) ~= 'table') then return end

    if (input.menu) then
        input.menu.resource = 'Unique_AdminPanel'
        input.menu.defaultSounds = Config.Sounds
        input.menu.hidden = MenuV_Hidden
    end
...
-- MenuV.Loaded is set by the NUI 'loaded' callback (bottom of file)
...
-- =====================================================================
-- Glue code (was the standalone `menuv` resource's own client script)
-- =====================================================================

local function ensure(v, d) return Utilities:Ensure(v, d) end
...
RegisterNUICallback('sound', function(info, cb)
    cb('ok')
    local key = string.upper(ensure(info.key, 'UNKNOWN'))
    local sound = ensure(Config.Sounds[key], {})
    if (string.lower(ensure(sound.type, 'unknown')) == 'native') then
        PlaySoundFrontend(-1, ensure(sound.name, 'UNKNOWN'), ensure(sound.library, 'UNKNOWN'), true)
    end
end)

for _, name in ipairs({ 'submit', 'close', 'switch', 'update', 'open', 'opened', 'close_all' }) do
    RegisterNUICallback(name, function(info, cb)
        local handler = MenuV.NUICallbacks[name]
        if (handler == nil) then cb('ok') return end
        handler(info, cb)
    end)
end

-- Key bindings (arrows / enter / backspace / +) ----------------------------

local NuiKeys = {}

local function RegisterNuiKey(action, description, defaultType, defaultKey, prefix)
    NuiKeys[action] = NuiKeys[action] or false
    local cmd = ('uapmenu_%s%s'):format(prefix, string.lower(action))
    RegisterKeyMapping('+' .. cmd, description, defaultType, defaultKey)
    RegisterCommand('+' .. cmd, function()
        -- only bother the NUI while a menu is actually open
        if (MenuV.CurrentMenu ~= nil and not MenuV_Hidden) then
            SendNUIMessage({ action = 'KEY_PRESSED', key = action, _menuv = true })
        end
    end, false)
    RegisterCommand('-' .. cmd, function()
        if (MenuV.CurrentMenu ~= nil and not MenuV_Hidden) then
            SendNUIMessage({ action = 'KEY_RELEASED', key = action, _menuv = true })
        end
    end, false)
end

RegisterNuiKey('UP', 'Admin menu - Up', 'KEYBOARD', 'UP', '')
RegisterNuiKey('DOWN', 'Admin menu - Down', 'KEYBOARD', 'DOWN', '')
RegisterNuiKey('LEFT', 'Admin menu - Left', 'KEYBOARD', 'LEFT', '')
RegisterNuiKey('RIGHT', 'Admin menu - Right', 'KEYBOARD', 'RIGHT', '')
RegisterNuiKey('ENTER', 'Admin menu - Select', 'KEYBOARD', 'RETURN', '')
RegisterNuiKey('CLOSE', 'Admin menu - Back', 'KEYBOARD', 'BACK', '')
RegisterNuiKey('CLOSE_ALL', 'Admin menu - Close all', 'KEYBOARD', 'PLUS', '')
RegisterNuiKey('UP', 'Admin menu (pad) - Up', 'PAD_ANALOGBUTTON', 'LUP_INDEX', 'pad_')
RegisterNuiKey('DOWN', 'Admin menu (pad) - Down', 'PAD_ANALOGBUTTON', 'LDOWN_INDEX', 'pad_')
RegisterNuiKey('LEFT', 'Admin menu (pad) - Left', 'PAD_ANALOGBUTTON', 'LLEFT_INDEX', 'pad_')
RegisterNuiKey('RIGHT', 'Admin menu (pad) - Right', 'PAD_ANALOGBUTTON', 'LRIGHT_INDEX', 'pad_')
RegisterNuiKey('ENTER', 'Admin menu (pad) - Select', 'PAD_ANALOGBUTTON', 'RDOWN_INDEX', 'pad_')
RegisterNuiKey('CLOSE', 'Admin menu (pad) - Back', 'PAD_ANALOGBUTTON', 'RRIGHT_INDEX', 'pad_')
RegisterNuiKey('CLOSE_ALL', 'Admin menu (pad) - Close all', 'PAD_ANALOGBUTTON', 'R3_INDEX', 'pad_')

-- Hide the menu while the pause menu is open / screen is faded ----------------
-- Only polls while a menu is open; sleeps longer when idle (was a constant
-- Wait(0)+Wait(250) loop in upstream).
CreateThread(function()
    while true do
        if (MenuV.CurrentMenu ~= nil and MenuV.Loaded) then
            local hidden = IsScreenFadedOut() or IsPauseMenuActive()
            if (MenuV_Hidden ~= hidden) then
                MenuV_Hidden = hidden
                SendNUIMessage({ action = 'UPDATE_STATUS', status = not hidden, _menuv = true })
            end
            Wait(250)
        else
            Wait(1000)
...
-- The one thing that leaves the private environment.
_G.MenuV = MenuV
