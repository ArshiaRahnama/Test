...
RegisterNetEvent('Unique_AdminPanel:ApplySetHealth')
AddEventHandler('Unique_AdminPanel:ApplySetHealth', function(pct)
...
RegisterNetEvent('Unique_AdminPanel:ApplySetArmor')
AddEventHandler('Unique_AdminPanel:ApplySetArmor', function(pct)
...
local Cuffed = false
RegisterNetEvent('Unique_AdminPanel:ApplyCuff')
AddEventHandler('Unique_AdminPanel:ApplyCuff', function()
    Cuffed = not Cuffed
    local ped = PlayerPedId()
...
RegisterNetEvent('Unique_AdminPanel:ShowScreenshot')
AddEventHandler('Unique_AdminPanel:ShowScreenshot', function(dataUri, targetName)
    SendNUIMessage({ type = 'screenshot', data = dataUri, name = targetName })
    SetNuiFocus(true, true)
    InAdminNui = true
end)

RegisterNetEvent('Unique_AdminPanel:ApplyGivenVehicle')
AddEventHandler('Unique_AdminPanel:ApplyGivenVehicle', function(model)
...
RegisterNetEvent('Unique_AdminPanel:PlayAnnounceSound')
AddEventHandler('Unique_AdminPanel:PlayAnnounceSound', function()
...
function OpenOnlinePlayersPanel()
    ESX.TriggerServerCallback('Unique_AdminPanel:GetOnlinePlayers', function(list)
...
function OpenNewPlayersPanel()
    ESX.TriggerServerCallback('Unique_AdminPanel:GetNewPlayers', function(list)
...
function OpenDashboardPanel()
    ESX.TriggerServerCallback('Unique_AdminPanel:GetDashboard', function(data)
        if not data then
...
RegisterNetEvent('Unique_AdminPanel:ApplyFreezeTime')
AddEventHandler('Unique_AdminPanel:ApplyFreezeTime', function(state)
    TimeFrozen = state
    if state then
...
RegisterNetEvent('Unique_AdminPanel:ApplyTrafficDensity')
AddEventHandler('Unique_AdminPanel:ApplyTrafficDensity', function(level)
    TrafficLevel = level
...
RegisterNetEvent('Unique_AdminPanel:ApplyLaunch')
AddEventHandler('Unique_AdminPanel:ApplyLaunch', function()
    local ped = PlayerPedId()
    ClearPedTasksImmediately(ped)
...
function SetCurrentVehicleLivery(livery)
    ...
    TriggerServerEvent('Unique_AdminPanel:LogClientAction', "set-livery", ("plate: %s | livery: %s"):format(GetVehicleNumberPlateText(vehicle), livery))
end

-- ------------------------------------------------ JAIL / CS PERMISSION ---
-- The server already validated btn_jail/btn_cs (Unique_AdminPanel:RequestJail
...
RegisterNetEvent('Unique_AdminPanel:ProceedJail')
AddEventHandler('Unique_AdminPanel:ProceedJail', function(targetId, minutes, reason)
    TriggerServerEvent('arshia_jail:sendto', targetId, 'admin', minutes, reason)
end)

RegisterNetEvent('Unique_AdminPanel:ProceedCS')
AddEventHandler('Unique_AdminPanel:ProceedCS', function(targetId, count, reason)
