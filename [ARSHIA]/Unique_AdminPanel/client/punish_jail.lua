...
local function IsJobAllowed(jobname, jobgrade)
	for _, job in pairs(PunishConfig.AllowedJobs) do
...
function UnJail()
    ...
    TriggerServerEvent('Unique_Punishment:AntiCheatExempt', 5000, { teleport = true, speed = true })
    local releaseCoords = (type(sentence.unjail) == "table" or type(sentence.unjail) == "vector3") and sentence.unjail or PunishConfig.AdminJail.unjail
...
AddEventHandler("arshia_jail:factionjail", function(target)
	...
	if IsJobAllowed(PlayerData.job.name) then
        ...
        for _ , v in pairs(PunishConfig.CanJail) do
...
function playCutscene()
    ...
    Wait(3000)
    TriggerServerEvent('Unique_Punishment:AntiCheatExempt', 5000, { teleport = true, speed = true })
    SetEntityCoords(ped, PunishConfig.cutscene.cuff)
    Wait(500)
    TriggerServerEvent('Unique_Punishment:AntiCheatExempt', 5000, { teleport = true, speed = true })
    SetEntityCoords(ped, PunishConfig.cutscene.cuff)
    RequestModel(PunishConfig.cutscene.guardModel)
    while not HasModelLoaded(PunishConfig.cutscene.guardModel) do
        Wait(0)
    end
    RequestModel(PunishConfig.cutscene.clotheModel)
    ...
    local byped = CreatePed(4, PunishConfig.cutscene.guardModel, PunishConfig.cutscene.guardCoords.x, PunishConfig.cutscene.guardCoords.y, PunishConfig.cutscene.guardCoords.z, PunishConfig.cutscene.guardCoords.w,false)
    ...
    createCam(PunishConfig.cutscene.camCoords, PunishConfig.cutscene.camRot)
    DoScreenFadeIn(500)

    TaskGoStraightToCoord(byped, PunishConfig.cutscene.stopTurn.xyz, 1.0, 2500, PunishConfig.cutscene.stopTurn.w, 0)
    Wait(2500)
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.enterCoords, 1.0, 2000, 160.0, 0)
    Wait(2000)
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.clotheCoords2.xyz, 1.0, 3000, PunishConfig.cutscene.clotheCoords2.w, 0)
    Wait(3000)
    ...
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.stopNLook.xyz, 1.0, 1500, 85.28, 0)
    Wait(2000)
    RequestAnimDict('mp_prison_break')
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.computerCoords.xyz, 1.0, 2000, PunishConfig.cutscene.computerCoords.w, 0)
    Wait(2000)
    ...
    Wait(200)
    TaskGoStraightToCoord(ped, PunishConfig.cutscene.enterCoords, 1.0, 4000, PunishConfig.cutscene.enterHeadings.Front, 0)
    Wait(2000)
    ClearPedTasksImmediately(byped)
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.computerCoords.xyz, 1.0, 2500, PunishConfig.cutscene.computerCoords.w, 0)
    Wait(2500)
    TaskPlayAnim(byped, "mp_prison_break", "hack_loop", 8.0, 8.0, -1, 1, 1, 0, 0, 0)
    ...
    Wait(5500)
    FreezeEntityPosition(ped, false)
    TaskAchieveHeading(ped, PunishConfig.cutscene.enterHeadings.Side, 3000)
    Wait(3000)
    TaskPlayAnim(ped, "mp_character_creation@customise@male_a", "loop", 8.0, 8.0, -1, 1, 1, 0, 0, 0)
    FreezeEntityPosition(ped, true)
    Wait(6000)
    ...
    ClearPedTasksImmediately(byped)
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.stopNLook.xyz, 1.0, 2000, PunishConfig.cutscene.stopNLook.w, 0)
    Wait(2000)
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.grabCoords.xyz, 1.0, 5500, PunishConfig.cutscene.grabCoords.w, 0)
    ...
    Wait(1500)
    TaskGoStraightToCoord(byped, PunishConfig.cutscene.walkCoords, 1.0, 5500, 100, 0)
    Wait(5500)
    DetachEntity(ped, true, false)
    ClearPedSecondaryTask(ped)
    ClearPedSecondaryTask(byped)
    ...
    createCam(PunishConfig.cutscene.camCoords2, PunishConfig.cutscene.camRot2)
    TriggerServerEvent('Unique_Punishment:AntiCheatExempt', 5000, { teleport = true, speed = true })
    ESX.Game.Teleport(ped, PunishConfig.cutscene.spawnCoords2, function()
        RequestModel('s_m_y_swat_01')
        while not HasModelLoaded('s_m_y_swat_01') do
            Wait(0)
        end
        local swat = CreatePed(4, 's_m_y_swat_01', PunishConfig.cutscene.police2Coords.x, PunishConfig.cutscene.police2Coords.y, PunishConfig.cutscene.police2Coords.z, PunishConfig.cutscene.police2Coords.w,false)
        SetBlockingOfNonTemporaryEvents(swat, true)
        SetEntityHeading(swat, PunishConfig.cutscene.police2Coords.w)





        ESX.Game.SpawnLocalVehicle('riot', PunishConfig.cutscene.riotCoords.xyz, PunishConfig.cutscene.riotCoords.w, function(vehicle)
            local driver = CreatePed(1, 's_m_y_swat_01', PunishConfig.cutscene.police2Coords.x, PunishConfig.cutscene.police2Coords.y, PunishConfig.cutscene.police2Coords.z, PunishConfig.cutscene.police2Coords.w,false)
            ...
            SetTimeout(5000, function()
                createCam(PunishConfig.cutscene.camCoords3, PunishConfig.cutscene.camRot3)
            end)
