...
-- UnbanPlayer (server/ac_core.lua) server-side; temp bans just get deactivated.
...
function OpenImpoundYard()
    ...
    ESX.TriggerServerCallback('Unique_AdminPanel:SearchImpoundYard', function(rows)
        rows = rows or {}
        local options = {}
            ...
            options[#options + 1] = {
                ...
                onSelect = function()
                    ...
                    if alert == 'confirm' then
                        TriggerServerEvent('Unique_AdminPanel:ReleaseImpound', r.id)
...
function OpenFactionAudit()
    ESX.TriggerServerCallback('Unique_AdminPanel:GetFactionAccounts', function(accounts)
                ...
                options = {
                    {
                        ...
                        iconColor = '#5fae72',
                        onSelect = function()
                            ESX.TriggerServerCallback('Unique_AdminPanel:GetFactionHistory', function(points)
                    ...
                    },
                    {
                        ...
                        onSelect = function()
                            ...
                            if amount then
                                TriggerServerEvent('Unique_AdminPanel:AddFactionMoney', a.account_name, amount, reason)
                            end
                        end,
                    },
                },
...
function OpenCharacterTransfer()
    ...
    TriggerServerEvent('Unique_AdminPanel:TransferCharacter', srcId, destId)
end
...
function ShowPunishmentConfirm(targetId, actionLabel, onConfirm)
    ESX.TriggerServerCallback('Unique_AdminPanel:GetConfirmSummary', function(data)
        if not data then
...
function OpenBanSearch()
    ...
    ESX.TriggerServerCallback('Unique_AdminPanel:SearchBans', function(rows)
        rows = rows or {}
        local options = {}
...
function OpenDutyHistory()
    ...
    ESX.TriggerServerCallback('Unique_AdminPanel:GetAdminNameList', function(names)
        names = names or {}
        ...
        ESX.TriggerServerCallback('Unique_AdminPanel:GetDutyHistory', function(rows)
            rows = rows or {}
            local options = {}
...
RegisterNUICallback('setButtonPerm', function(payload, cb)
    if payload and payload.id then
        TriggerServerEvent('Unique_AdminPanel:SetButtonPerm', payload.id, payload.level)
    end
    cb('ok')
end)
...
function OpenBanAppeals()
    ESX.TriggerServerCallback('Unique_AdminPanel:GetPendingAppeals', function(rows)
        rows = rows or {}
        local options = {}
                ...
                options = {
                    {
                        ...
                        iconColor = '#5fae72',
                        onSelect = function()
                            TriggerServerEvent('Unique_AdminPanel:ReviewAppeal', a.id, true)
                    ...
                    },
                    {
                        ...
                        iconColor = '#c85450',
                        onSelect = function()
                            TriggerServerEvent('Unique_AdminPanel:ReviewAppeal', a.id, false)
                            Citizen.SetTimeout(500, OpenBanAppeals)
                        end,
                    },
                },
...
function OpenEconomyChart()
    ESX.TriggerServerCallback('Unique_AdminPanel:GetEconomyHistory', function(points)
...
RegisterNUICallback('radialAction', function(payload, cb)
    ...
    elseif action == 'heal' then
        TriggerServerEvent('Unique_AdminPanel:HealPlayer', GetPlayerServerId(PlayerId()))
    elseif action == 'revive' then
        TriggerServerEvent('Unique_AdminPanel:RevivePlayer', GetPlayerServerId(PlayerId()))
    elseif action == 'spawncar' then
        TriggerServerEvent('Unique_AdminPanel:SpawnVehicle', 'adder', '')
...
Citizen.CreateThread(function()
    while true do
        if aduty then
            ESX.TriggerServerCallback('Unique_AdminPanel:GetServerStats', function(stats)
                if stats then
