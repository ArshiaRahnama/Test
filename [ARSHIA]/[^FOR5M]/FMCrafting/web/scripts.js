...
window.addEventListener('message', function(event) {
				...
				if(true){
							...
							<div class="col-md-3">
								<div class="card item-card" id="${itemId}wb_${event.data.wb}">
									<div class="card-body item-card-body d-flex justify-content-center align-items-center" style="background-color: rgba(220, 220, 220, 0.20)" id="selected${itemId}wb_${event.data.wb}">
										<span class="item-title text-center">${itemId}</span>
										<img src="`+inventory+`/${itemId}.png" class="image">
									</div>
								</div>
							</div>
						`;
					...
					if(!myEle) {
						$(document).on('click', "#"+itemId+"wb_"+event.data.wb, function() {
							...
							sound.volume = 0.3;
							sound.play();
							$.post('https://FMCrafting/action', JSON.stringify({
								action: "craft",
								item: id,
								crafts: craft,
								itemName: itemName,
	...
	case 'openSideCraft':
		...
		var recipe = ``;
		var img = `
			<img src="`+inventory+`/${event.data.itemNameID}.png" class="image_itemselected">
			<span style="color: #fff; font-weight: 600; font-size: 20px;" class="ms-2">${event.data.itemNameID} x${event.data.itemAmount}</span>
		`;
			...
			if (event.data.recipe[i][2]){
				recipe += `
					<div class="d-flex align-items-center mx-1">
						<img src="`+inventory+`/${event.data.recipe[i][0]}.png" class="image_components">
						<span style="color: #fff; font-weight: 600; font-size: 20px;" class="ms-2">${idName} x${event.data.recipe[i][1]} (${event.data.inventory[i].key})</span>
					</div>
				`;
			} else {
				recipe += `
		...
		if(canCraft) {
			$('#craft-button-div').html(`
				<button type="button" id="craft-button" data-item="${event.data.itemNameID}" data-recipe="${event.data.recipe}" data-amount="${event.data.itemAmount}" onclick="craft(this)" class="btn btn-blue flex-grow-1 mt-2" style="border-radius: 10px; flex-basis: 100%; width: 100%; font-weight: 600;"><i class="fas fa-pencil-ruler"></i> CRAFT [${event.data.itemNameID}]</button>
			`);
			$('#craft-button').fadeIn();
		} else {
